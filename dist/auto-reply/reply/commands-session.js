...
import { scheduleGatewaySigusr1Restart, triggerClawdfunRestart } from "../../infra/restart.js";
...
export const handleRestartCommand = async (params, allowTextCommands) => {
    ...
    if (hasSigusr1Listener) {
        ...
        return {
            shouldContinue: false,
            reply: {
                text: "⚙️ Restarting clawdfun in-process (SIGUSR1); back in a few seconds.",
            },
        };
    }
    const restartMethod = triggerClawdfunRestart();
    if (!restartMethod.ok) {
    ...
    return {
        shouldContinue: false,
        reply: {
            text: `⚙️ Restarting clawdfun via ${restartMethod.method}; give me a few seconds to come back online.`,
        },
    };
};
