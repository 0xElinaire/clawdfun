...
export function hasAudioTranscriptionConfig(cfg) {
    if (cfg.tools?.audio?.transcription?.args?.length)
        return true;
    return Boolean(cfg.audio?.transcription?.command?.length);
}
export async function transcribeInboundAudio(cfg, ctx, runtime) {
    ...
    const timeoutMs = Math.max((toolTranscriber?.timeoutSeconds ?? legacyTranscriber?.timeoutSeconds ?? 45) * 1000, 1_000);
    let tmpPath;
    ...
    try {
        if (!mediaPath && ctx.MediaUrl) {
            ...
            tmpPath = path.join(os.tmpdir(), `clawdfun-audio-${crypto.randomUUID()}.ogg`);
            await fs.writeFile(tmpPath, buffer);
            mediaPath = tmpPath;
            if (shouldLogVerbose()) {
                logVerbose(`Downloaded audio for transcription (${(buffer.length / (1024 * 1024)).toFixed(2)}MB) -> ${tmpPath}`);
            }
        }
        if (!mediaPath)
            return undefined;
        const templCtx = { ...ctx, MediaPath: mediaPath };
        const argv = hasToolTranscriber
            ? [AUDIO_TRANSCRIPTION_BINARY, ...(toolTranscriber?.args ?? [])].map((part, index) => index === 0 ? part : applyTemplate(part, templCtx))
