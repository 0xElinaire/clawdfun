...
export function collectSyncedFolderFindings(params) {
    ...
    if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
        findings.push({
            ...
            remediation: `Keep CLAWDFUN_STATE_DIR on a local-only volume and re-run "${formatCliCommand("clawdfun security audit --fix")}".`,
        });
    }
    return findings;
}
...
export function collectSecretsInConfigFindings(cfg) {
    ...
    if (password && !looksLikeEnvRef(password)) {
        findings.push({
            ...
            remediation: "Prefer CLAWDFUN_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
        });
    }
    ...
    if (browserToken && !looksLikeEnvRef(browserToken)) {
        findings.push({
            ...
            remediation: "Prefer CLAWDFUN_BROWSER_CONTROL_TOKEN (env) and remove browser.controlToken from disk.",
        });
    }
...
export function collectHooksHardeningFindings(cfg) {
    ...
    const browserToken = typeof cfg.browser?.controlToken === "string" && cfg.browser.controlToken.trim()
        ...
        : process.env.CLAWDFUN_BROWSER_CONTROL_TOKEN?.trim() || null;
