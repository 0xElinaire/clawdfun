    ...
    if (stateDirStat.ok) {
        ...
        if (isWorldWritable(bits)) {
            findings.push({
                ...
                detail: `${params.stateDir} mode=${formatOctal(bits)}; other users can write into your Clawdfun state.`,
            ...
            findings.push({
                ...
                detail: `${params.stateDir} mode=${formatOctal(bits)}; group users can write into your Clawdfun state.`,
                remediation: `chmod 700 ${params.stateDir}`,
            });
        }
...
function collectBrowserControlFindings(cfg) {
    ...
    try {
        ...
        findings.push({
            ...
            detail: String(err),
            remediation: `Fix browser.controlUrl/browser.cdpUrl in ${resolveConfigPath()} and re-run "${formatCliCommand("clawdfun security audit --deep")}".`,
        });
        return findings;
    }
    if (!resolved.enabled)
    ...
    const envToken = process.env.CLAWDFUN_BROWSER_CONTROL_TOKEN?.trim();
    ...
    if (!isLoopback) {
        if (!controlToken) {
            findings.push({
                ...
                detail: `browser.controlUrl is non-loopback (${resolved.controlUrl}) but no browser.controlToken (or CLAWDFUN_BROWSER_CONTROL_TOKEN) is configured.`,
                remediation: "Set browser.controlToken (or export CLAWDFUN_BROWSER_CONTROL_TOKEN) and prefer serving over Tailscale Serve or HTTPS reverse proxy.",
            });
        }
    ...
    const resolveAuth = (mode) => {
        ...
        const token = mode === "remote"
            ? typeof remote?.token === "string" && remote.token.trim()
                ? remote.token.trim()
                : undefined
            : process.env.CLAWDFUN_GATEWAY_TOKEN?.trim() ||
                (typeof authToken === "string" && authToken.trim() ? authToken.trim() : undefined);
        const password = process.env.CLAWDFUN_GATEWAY_PASSWORD?.trim() ||
            (mode === "remote"
...
export async function runSecurityAudit(opts) {
    ...
    if (deep?.gateway?.attempted && deep.gateway.ok === false) {
        findings.push({
            ...
            remediation: `Run "${formatCliCommand("clawdfun status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("clawdfun security audit --deep")}".`,
        });
    }
