...
async function ensureClawdfunExtensions(manifest) {
    const extensions = manifest.clawdfun?.extensions;
    if (!Array.isArray(extensions)) {
        throw new Error("package.json missing clawdfun.extensions");
    }
    const list = extensions.map((e) => (typeof e === "string" ? e.trim() : "")).filter(Boolean);
    if (list.length === 0) {
        throw new Error("package.json clawdfun.extensions is empty");
    }
    return list;
}
...
async function installPluginFromPackageDir(params) {
    ...
    let extensions;
    try {
        extensions = await ensureClawdfunExtensions(manifest);
    }
    catch (err) {
...
export async function installPluginFromArchive(params) {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "clawdfun-plugin-"));
...
export async function installPluginFromNpmSpec(params) {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "clawdfun-npm-pack-"));
