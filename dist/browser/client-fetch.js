...
function getBrowserControlToken() {
    const env = process.env.CLAWDFUN_BROWSER_CONTROL_TOKEN?.trim();
    if (env)
        return env;
...
function enhanceBrowserFetchError(url, err, timeoutMs) {
    ...
    const hint = `Start (or restart) the Clawdfun gateway (Clawdfun.app menubar, or \`${formatCliCommand("clawdfun gateway")}\`) and try again.`;
