...
import { DEFAULT_CLAWD_BROWSER_COLOR, DEFAULT_CLAWD_BROWSER_CONTROL_URL, DEFAULT_CLAWD_BROWSER_ENABLED, DEFAULT_BROWSER_DEFAULT_PROFILE_NAME, DEFAULT_CLAWD_BROWSER_PROFILE_NAME, } from "./constants.js";
 ...
 *
 * Note: this is a Clawdfun browser profile (routing config), not a Chrome user profile.
...
export function resolveBrowserConfig(cfg) {
    ...
    const envControlUrl = process.env.CLAWDFUN_BROWSER_CONTROL_URL?.trim();
    const controlToken = cfg?.controlToken?.trim() || undefined;
    const derivedControlPort = (() => {
        const raw = process.env.CLAWDFUN_GATEWAY_PORT?.trim();
        if (!raw)
            return null;
    ...
    const controlInfo = parseHttpUrl(cfg?.controlUrl ?? envControlUrl ?? derivedControlUrl ?? DEFAULT_CLAWD_BROWSER_CONTROL_URL, "browser.controlUrl");
    ...
    if (rawCdpUrl) {
        ...
        if (derivedPort > 65535) {
            throw new Error(`browser.controlUrl port (${controlPort}) is too high; cannot derive CDP port (${derivedPort})`);
        }
...
export function shouldStartLocalBrowserServer(resolved) {
