...
function normalizePluginHookEntry(api, entry) {
    return {
        ...
        hook: {
            ...entry.hook,
            source: "clawdfun-plugin",
            pluginId: api.id,
        },
        clawdfun: {
            ...entry.clawdfun,
            hookKey: entry.clawdfun?.hookKey ?? `${api.id}:${entry.hook.name}`,
...
async function loadHookHandler(entry, api) {
    try {
        ...
        const exportName = entry.clawdfun?.export ?? "default";
...
export async function registerPluginHooksFromDir(api, dir) {
    ...
    for (const entry of hooks) {
        ...
        const events = normalizedEntry.clawdfun?.events ?? [];
        if (events.length === 0) {
