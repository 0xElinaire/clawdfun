...
import { CONFIG_PATH_CLAWDFUN, loadConfig, readConfigFileSnapshot, resolveGatewayPort, validateConfigObjectWithPlugins, writeConfigFile, } from "../config/config.js";
...
export async function runGmailSetup(opts) {
    ...
    if (!configSnapshot.valid) {
        throw new Error(`Config invalid: ${CONFIG_PATH_CLAWDFUN}`);
    }
    ...
    defaultRuntime.log(`- config: ${displayPath(CONFIG_PATH_CLAWDFUN)}`);
    defaultRuntime.log(`Next: ${formatCliCommand("clawdfun webhooks gmail run")}`);
}
