...
function resolveHookKey(entry) {
    return entry.clawdfun?.hookKey ?? entry.hook.name;
}
...
function buildHookStatus(entry, config, eligibility) {
    ...
    const managedByPlugin = entry.hook.source === "clawdfun-plugin";
    const disabled = managedByPlugin ? false : hookConfig?.enabled === false;
    const always = entry.clawdfun?.always === true;
    const emoji = entry.clawdfun?.emoji ?? entry.frontmatter.emoji;
    const homepageRaw = entry.clawdfun?.homepage ??
        entry.frontmatter.homepage ??
        entry.frontmatter.website ??
        entry.frontmatter.url;
    const homepage = homepageRaw?.trim() ? homepageRaw.trim() : undefined;
    const events = entry.clawdfun?.events ?? [];
    const requiredBins = entry.clawdfun?.requires?.bins ?? [];
    const requiredAnyBins = entry.clawdfun?.requires?.anyBins ?? [];
    const requiredEnv = entry.clawdfun?.requires?.env ?? [];
    const requiredConfig = entry.clawdfun?.requires?.config ?? [];
    const requiredOs = entry.clawdfun?.os ?? [];
