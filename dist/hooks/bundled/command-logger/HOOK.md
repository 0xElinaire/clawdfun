    ...
    "clawdfun":
      {
        "emoji": "📝",
        "events": ["command"],
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with Clawdfun" }],
      },
  }
---
...
2. **Appends to log file** - Writes a JSON line to `~/.clawdfun/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.clawdfun/logs/commands.log ~/.clawdfun/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/clawdfun`:
   ```
   /home/username/.clawdfun/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
