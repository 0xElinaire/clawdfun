...
async function ensureClawdfunHooks(manifest) {
    const hooks = manifest.clawdfun?.hooks;
    if (!Array.isArray(hooks)) {
        throw new Error("package.json missing clawdfun.hooks");
    }
    const list = hooks.map((e) => (typeof e === "string" ? e.trim() : "")).filter(Boolean);
    if (list.length === 0) {
        throw new Error("package.json clawdfun.hooks is empty");
    }
    return list;
}
...
async function installHookPackageFromDir(params) {
    ...
    let hookEntries;
    try {
        hookEntries = await ensureClawdfunHooks(manifest);
    }
    catch (err) {
...
export async function installHooksFromArchive(params) {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "clawdfun-hook-"));
...
export async function installHooksFromNpmSpec(params) {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "clawdfun-hook-pack-"));
