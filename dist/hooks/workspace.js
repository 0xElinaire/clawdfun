...
import { parseFrontmatter, resolveClawdfunMetadata, resolveHookInvocationPolicy, } from "./frontmatter.js";
...
function resolvePackageHooks(manifest) {
    const raw = manifest.clawdfun?.hooks;
...
export function loadHookEntriesFromDir(params) {
    ...
    return hooks.map((hook) => {
        ...
        const entry = {
            ...
            frontmatter,
            clawdfun: resolveClawdfunMetadata(frontmatter),
...
function loadHookEntries(workspaceDir, opts) {
    ...
    const extraHooks = extraDirs.flatMap((dir) => {
        ...
        return loadHooksFromDir({
            dir: resolved,
            source: "clawdfun-workspace", // Extra dirs treated as workspace
        });
    });
    const managedHooks = loadHooksFromDir({
        dir: managedHooksDir,
        source: "clawdfun-managed",
    });
    const workspaceHooks = loadHooksFromDir({
        dir: workspaceHooksDir,
        source: "clawdfun-workspace",
    });
