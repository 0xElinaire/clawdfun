...
function formatAuthIssueHint(issue) {
    ...
    if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
        return `Run \`codex login\` (or \`${formatCliCommand("clawdfun configure")}\` → OpenAI Codex OAuth).`;
    }
    return `Re-auth via \`${formatCliCommand("clawdfun configure")}\` or \`${formatCliCommand("clawdfun onboard")}\`.`;
}
