...
export function formatGatewayProvidersStatusLines(payload) {
    const lines = [];
    ...
    const accountLines = (provider, accounts) => accounts.map((account) => {
        ...
        const inboundAt = typeof account.lastInboundAt === "number" &&
            Number.isFinite(account.lastInboundAt)
            ? account.lastInboundAt
            : null;
        const outboundAt = typeof account.lastOutboundAt === "number" &&
        ...
        if (typeof account.botTokenSource === "string" &&
            account.botTokenSource) {
            bits.push(`bot:${account.botTokenSource}`);
        }
        if (typeof account.appTokenSource === "string" &&
    ...
    if (issues.length > 0) {
        ...
        lines.push(`- Run: clawdfun doctor`);
        lines.push("");
    }
...
async function formatConfigProvidersStatusLines(cfg, meta) {
    const lines = [];
...
export async function providersStatusCommand(opts, runtime = defaultRuntime) {
    ...
    try {
        ...
        runtime.log(formatGatewayProvidersStatusLines(payload).join("\n"));
    }
    catch (err) {
        ...
        runtime.log((await formatConfigProvidersStatusLines(cfg, {
            path: snapshot.path,
            mode,
        })).join("\n"));
    }
