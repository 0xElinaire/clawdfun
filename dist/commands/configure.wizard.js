    ...
    return guardCancel(await select({
        ...
        options: [
            ...
            },
            {
                ...
                hint: "Delete channel tokens/settings from clawdfun.json",
            },
        ],
...
export async function runConfigureWizard(opts, runtime = defaultRuntime) {
    try {
        ...
        intro(opts.command === "update" ? "Clawdfun update wizard" : "Clawdfun configure");
        ...
        if (snapshot.exists) {
            ...
            if (!snapshot.valid) {
                outro(`Config invalid. Run \`${formatCliCommand("clawdfun doctor")}\` to repair it, then re-run configure.`);
                runtime.exit(1);
                return;
            }
        }
        ...
        const localProbe = await probeGatewayReachable({
            url: localUrl,
            token: baseConfig.gateway?.auth?.token ?? process.env.CLAWDFUN_GATEWAY_TOKEN,
            password: baseConfig.gateway?.auth?.password ?? process.env.CLAWDFUN_GATEWAY_PASSWORD,
        });
        ...
        let gatewayToken = nextConfig.gateway?.auth?.token ??
            ...
            process.env.CLAWDFUN_GATEWAY_TOKEN;
        ...
        if (opts.sections) {
            ...
            if (selected.includes("health")) {
                ...
                const token = nextConfig.gateway?.auth?.token ?? process.env.CLAWDFUN_GATEWAY_TOKEN;
                const password = nextConfig.gateway?.auth?.password ?? process.env.CLAWDFUN_GATEWAY_PASSWORD;
        ...
        const newPassword = nextConfig.gateway?.auth?.password ?? process.env.CLAWDFUN_GATEWAY_PASSWORD;
        const oldPassword = baseConfig.gateway?.auth?.password ?? process.env.CLAWDFUN_GATEWAY_PASSWORD;
        const token = nextConfig.gateway?.auth?.token ?? process.env.CLAWDFUN_GATEWAY_TOKEN;
