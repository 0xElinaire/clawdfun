...
import { CONFIG_PATH_CLAWDFUN, readConfigFileSnapshot, writeConfigFile } from "../config/config.js";
...
export async function doctorCommand(runtime = defaultRuntime, options = {}) {
    ...
    const configPath = configResult.path ?? CONFIG_PATH_CLAWDFUN;
    if (!cfg.gateway?.mode) {
        const lines = [
            "gateway.mode is unset; gateway start will be blocked.",
            `Fix: run ${formatCliCommand("clawdfun configure")} and set Gateway mode (local/remote).`,
            `Or set directly: ${formatCliCommand("clawdfun config set gateway.mode local")}`,
        ];
        if (!fs.existsSync(configPath)) {
            lines.push(`Missing config: run ${formatCliCommand("clawdfun setup")} first.`);
        }
        note(lines.join("\n"), "Gateway");
    }
    ...
    await noteStateIntegrity(cfg, prompter, configResult.path ?? CONFIG_PATH_CLAWDFUN);
    ...
    if (shouldWriteConfig) {
        ...
        const backupPath = `${CONFIG_PATH_CLAWDFUN}.bak`;
        if (fs.existsSync(backupPath)) {
            runtime.log(`Backup: ${shortenHomePath(backupPath)}`);
        }
    }
    else {
        runtime.log(`Run "${formatCliCommand("clawdfun doctor --fix")}" to apply changes.`);
    }
