...
export async function noteMacLaunchAgentOverrides() {
    if (process.platform !== "darwin")
        return;
    const markerPath = path.join(resolveHomeDir(), ".clawdfun", "disable-launchagent");
...
export async function noteMacLaunchctlGatewayEnvOverrides(cfg, deps) {
    ...
    const envToken = await getenv("CLAWDFUN_GATEWAY_TOKEN");
    const envPassword = await getenv("CLAWDFUN_GATEWAY_PASSWORD");
    if (!envToken && !envPassword)
        return;
    const lines = [
        "- launchctl environment overrides detected (can cause confusing unauthorized errors).",
        envToken ? "- `CLAWDFUN_GATEWAY_TOKEN` is set; it overrides config tokens." : undefined,
        envPassword
            ? "- `CLAWDFUN_GATEWAY_PASSWORD` is set; it overrides config passwords."
            : undefined,
        "- Clear overrides and restart the app/gateway:",
        envToken ? "  launchctl unsetenv CLAWDFUN_GATEWAY_TOKEN" : undefined,
        envPassword ? "  launchctl unsetenv CLAWDFUN_GATEWAY_PASSWORD" : undefined,
    ].filter((line) => Boolean(line));
