    ...
    for (const root of roots) {
        ...
        for (const entry of entries) {
            ...
            if (entry.name.startsWith("."))
                continue;
            const candidate = path.resolve(root, entry.name, ".clawdfun");
...
export async function noteStateIntegrity(cfg, prompter, configPath) {
    ...
    const defaultStateDir = path.join(homedir(), ".clawdfun");
...
export function noteWorkspaceBackupTip(workspaceDir) {
    ...
    note([
        ...
        "- Keep ~/.clawdfun out of git; it contains credentials and session history.",
