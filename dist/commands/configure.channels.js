...
import { CONFIG_PATH_CLAWDFUN } from "../config/config.js";
...
export async function removeChannelConfigWizard(cfg, runtime) {
    ...
    while (true) {
        ...
        if (configured.length === 0) {
            note([
                "No channel config found in clawdfun.json.",
                `Tip: \`${formatCliCommand("clawdfun channels status")}\` shows what is configured and enabled.`,
            ].join("\n"), "Remove channel");
            return next;
        }
        ...
        const confirmed = guardCancel(await confirm({
            message: `Delete ${label} configuration from ${shortenHomePath(CONFIG_PATH_CLAWDFUN)}?`,
            initialValue: false,
        }), runtime);
        if (!confirmed)
            continue;
