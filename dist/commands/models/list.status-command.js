...
import { resolveClawdfunAgentDir } from "../../agents/agent-paths.js";
...
import { CONFIG_PATH_CLAWDFUN, loadConfig } from "../../config/config.js";
...
export async function modelsStatusCommand(opts, runtime) {
    ...
    const agentDir = resolveClawdfunAgentDir();
    ...
    if (opts.json) {
        runtime.log(JSON.stringify({
            configPath: CONFIG_PATH_CLAWDFUN,
            agentDir,
    ...
    runtime.log(`${label("Config")}${colorize(rich, theme.muted, ":")} ${colorize(rich, theme.info, shortenHomePath(CONFIG_PATH_CLAWDFUN))}`);
    ...
    if (missingProvidersInUse.length > 0) {
        ...
        for (const provider of missingProvidersInUse) {
            const hint = provider === "anthropic"
                ? `Run \`claude setup-token\` or \`${formatCliCommand("clawdfun configure")}\`.`
                : `Run \`${formatCliCommand("clawdfun configure")}\` or set an API key env var.`;
