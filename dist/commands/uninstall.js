...
async function removeMacApp(runtime, dryRun) {
    if (process.platform !== "darwin")
        return;
    await removePath("/Applications/Clawdfun.app", runtime, {
        dryRun,
        label: "/Applications/Clawdfun.app",
    });
}
    ...
    if (!hadExplicit) {
        ...
        const selection = await multiselectStyled({
            ...
            options: [
                ...
                { value: "state", label: "State + config", hint: "~/.clawdfun" },
                { value: "workspace", label: "Workspace", hint: "agent files" },
                {
                    value: "app",
                    label: "macOS app",
                    hint: "/Applications/Clawdfun.app",
                },
            ],
