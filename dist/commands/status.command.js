...
export async function statusCommand(opts, runtime) {
    ...
    runtime.log(theme.heading("Clawdfun status"));
    runtime.log("");
    ...
    runtime.log(theme.muted(`Full report: ${formatCliCommand("clawdfun security audit")}`));
    runtime.log(theme.muted(`Deep probe: ${formatCliCommand("clawdfun security audit --deep")}`));
    runtime.log("");
    ...
    runtime.log(`  Need to share?      ${formatCliCommand("clawdfun status --all")}`);
    runtime.log(`  Need to debug live? ${formatCliCommand("clawdfun logs --follow")}`);
    if (gatewayReachable) {
        runtime.log(`  Need to test channels? ${formatCliCommand("clawdfun status --deep")}`);
    }
    else {
        runtime.log(`  Fix reachability first: ${formatCliCommand("clawdfun gateway probe")}`);
    }
}
