...
function resolveTaskName(env) {
    const override = env.CLAWDFUN_WINDOWS_TASK_NAME?.trim();
    if (override)
        return override;
    return resolveGatewayWindowsTaskName(env.CLAWDFUN_PROFILE);
}
export function resolveTaskScriptPath(env) {
    const override = env.CLAWDFUN_TASK_SCRIPT?.trim();
    if (override)
        return override;
    const scriptName = env.CLAWDFUN_TASK_SCRIPT_NAME?.trim() || "gateway.cmd";
