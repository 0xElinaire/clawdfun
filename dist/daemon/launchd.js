...
import { formatGatewayServiceDescription, GATEWAY_LAUNCH_AGENT_LABEL, LEGACY_GATEWAY_LAUNCH_AGENT_LABELS, resolveGatewayLaunchAgentLabel, } from "./constants.js";
...
function resolveLaunchAgentLabel(args) {
    const envLabel = args?.env?.CLAWDFUN_LAUNCHD_LABEL?.trim();
    if (envLabel)
        return envLabel;
    return resolveGatewayLaunchAgentLabel(args?.env?.CLAWDFUN_PROFILE);
}
...
export function resolveGatewayLogPaths(env) {
    ...
    const prefix = env.CLAWDFUN_LOG_PREFIX?.trim() || "gateway";
    return {
        logDir,
...
export async function installLaunchAgent({ env, stdout, programArguments, workingDirectory, environment, description, }) {
    ...
    const serviceDescription = description ??
        formatGatewayServiceDescription({
            profile: env.CLAWDFUN_PROFILE,
            version: environment?.CLAWDFUN_SERVICE_VERSION ?? env.CLAWDFUN_SERVICE_VERSION,
        });
