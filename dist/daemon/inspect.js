...
function hasGatewayServiceMarker(content) {
    const lower = content.toLowerCase();
    return (lower.includes("clawdfun_service_marker") &&
        lower.includes(GATEWAY_SERVICE_MARKER.toLowerCase()) &&
        lower.includes("clawdfun_service_kind") &&
        lower.includes(GATEWAY_SERVICE_KIND.toLowerCase()));
}
function isClawdfunGatewayLaunchdService(label, contents) {
    if (hasGatewayServiceMarker(contents))
        return true;
    const lowerContents = contents.toLowerCase();
    if (!lowerContents.includes("gateway"))
        return false;
    return label.startsWith("com.clawdfun.");
}
function isClawdfunGatewaySystemdService(name, contents) {
    if (hasGatewayServiceMarker(contents))
        return true;
    if (!name.startsWith("clawdfun-gateway"))
        return false;
    return contents.toLowerCase().includes("gateway");
}
function isClawdfunGatewayTaskName(name) {
    const normalized = name.trim().toLowerCase();
    if (!normalized)
        return false;
    const defaultName = resolveGatewayWindowsTaskName().toLowerCase();
    return normalized === defaultName || normalized.startsWith("clawdfun gateway");
}
...
async function scanLaunchdDir(params) {
    ...
    for (const entry of entries) {
        ...
        if (isIgnoredLaunchdLabel(label))
            continue;
        if (isClawdfunGatewayLaunchdService(label, contents))
            continue;
        results.push({
            platform: "darwin",
            label,
...
async function scanSystemdDir(params) {
    ...
    for (const entry of entries) {
        ...
        if (!containsMarker(contents))
            continue;
        if (isClawdfunGatewaySystemdService(name, contents))
            continue;
        results.push({
            platform: "linux",
            label: entry,
...
export async function findExtraGatewayServices(env, opts = {}) {
    ...
    if (process.platform === "win32") {
        ...
        for (const task of tasks) {
            ...
            if (!name)
                continue;
            if (isClawdfunGatewayTaskName(name))
                continue;
