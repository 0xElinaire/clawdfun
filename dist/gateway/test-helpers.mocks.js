...
const testConfigRoot = {
    value: path.join(os.tmpdir(), `clawdfun-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};
export const setTestConfigRoot = (root) => {
    testConfigRoot.value = root;
    process.env.CLAWDFUN_CONFIG_PATH = path.join(root, "clawdfun.json");
};
...
vi.mock("../config/config.js", async () => {
    ...
    const resolveConfigPath = () => path.join(testConfigRoot.value, "clawdfun.json");
    ...
    return {
        ...actual,
        get CONFIG_PATH_CLAWDFUN() {
            return resolveConfigPath();
        },
        get STATE_DIR_CLAWDFUN() {
...
process.env.CLAWDFUN_SKIP_CHANNELS = "1";
process.env.CLAWDFUN_SKIP_CRON = "1";
