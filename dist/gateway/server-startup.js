...
export async function startGatewaySidecars(params) {
    ...
    if (!isTruthyEnvValue(process.env.CLAWDFUN_SKIP_GMAIL_WATCHER)) {
        try {
    ...
    // Tests can opt out via CLAWDFUN_SKIP_CHANNELS (or legacy CLAWDFUN_SKIP_PROVIDERS).
    const skipChannels = isTruthyEnvValue(process.env.CLAWDFUN_SKIP_CHANNELS) ||
        isTruthyEnvValue(process.env.CLAWDFUN_SKIP_PROVIDERS);
    if (!skipChannels) {
        try {
        ...
        params.logChannels.info("skipping channel start (CLAWDFUN_SKIP_CHANNELS=1 or CLAWDFUN_SKIP_PROVIDERS=1)");
    }
