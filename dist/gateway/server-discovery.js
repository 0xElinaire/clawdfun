...
export function formatBonjourInstanceName(displayName) {
    ...
    if (!trimmed)
        return "Clawdfun";
    if (/clawdfun/i.test(trimmed))
        return trimmed;
    return `${trimmed} (Clawdfun)`;
}
export function resolveBonjourCliPath(opts = {}) {
    const env = opts.env ?? process.env;
    const envPath = env.CLAWDFUN_CLI_PATH?.trim();
    if (envPath)
        return envPath;
    ...
    const siblingCli = path.join(execDir, "clawdfun");
    ...
    if (isFile(distCli))
        return distCli;
    const binCli = path.join(cwd, "bin", "clawdfun.js");
    if (isFile(binCli))
        return binCli;
    return undefined;
}
export async function resolveTailnetDnsHint(opts) {
    const env = opts?.env ?? process.env;
    const envRaw = env.CLAWDFUN_TAILNET_DNS?.trim();
