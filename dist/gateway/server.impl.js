...
import { CONFIG_PATH_CLAWDFUN, isNixMode, loadConfig, migrateLegacyConfig, readConfigFileSnapshot, writeConfigFile, } from "../config/config.js";
...
import { ensureClawdfunCliOnPath } from "../infra/path-env.js";
...
export async function startGatewayServer(port = 18789, opts = {}) {
    ...
    process.env.CLAWDFUN_GATEWAY_PORT = String(port);
    logAcceptedEnvOption({
        key: "CLAWDFUN_RAW_STREAM",
        description: "raw stream logging enabled",
    });
    logAcceptedEnvOption({
        key: "CLAWDFUN_RAW_STREAM_PATH",
    ...
    if (configSnapshot.legacyIssues.length > 0) {
        ...
        if (!migrated) {
            throw new Error(`Legacy config entries detected but auto-migration failed. Run "${formatCliCommand("clawdfun doctor")}" to migrate.`);
        }
    ...
    if (configSnapshot.exists && !configSnapshot.valid) {
        ...
        throw new Error(`Invalid config at ${configSnapshot.path}.\n${issues}\nRun "${formatCliCommand("clawdfun doctor")}" to repair, then retry.`);
    }
    ...
    const configReloader = startGatewayConfigReloader({
        ...
        watchPath: CONFIG_PATH_CLAWDFUN,
    });
