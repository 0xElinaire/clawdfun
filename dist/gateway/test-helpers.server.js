...
async function setupGatewayTestHome() {
    ...
    previousStateDir = process.env.CLAWDFUN_STATE_DIR;
    previousConfigPath = process.env.CLAWDFUN_CONFIG_PATH;
    previousSkipBrowserControl = process.env.CLAWDFUN_SKIP_BROWSER_CONTROL_SERVER;
    previousSkipGmailWatcher = process.env.CLAWDFUN_SKIP_GMAIL_WATCHER;
    previousSkipCanvasHost = process.env.CLAWDFUN_SKIP_CANVAS_HOST;
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "clawdfun-gateway-home-"));
    process.env.HOME = tempHome;
    process.env.USERPROFILE = tempHome;
    process.env.CLAWDFUN_STATE_DIR = path.join(tempHome, ".clawdfun");
    delete process.env.CLAWDFUN_CONFIG_PATH;
}
function applyGatewaySkipEnv() {
    process.env.CLAWDFUN_SKIP_BROWSER_CONTROL_SERVER = "1";
    process.env.CLAWDFUN_SKIP_GMAIL_WATCHER = "1";
    process.env.CLAWDFUN_SKIP_CANVAS_HOST = "1";
}
async function resetGatewayTestState(options) {
    ...
    tempConfigRoot = options.uniqueConfigRoot
        ? await fs.mkdtemp(path.join(tempHome, "clawdfun-test-"))
        : path.join(tempHome, ".clawdfun-test");
...
async function cleanupGatewayTestHome(options) {
    ...
    if (options.restoreEnv) {
        ...
        if (previousStateDir === undefined)
            delete process.env.CLAWDFUN_STATE_DIR;
        else
            process.env.CLAWDFUN_STATE_DIR = previousStateDir;
        if (previousConfigPath === undefined)
            delete process.env.CLAWDFUN_CONFIG_PATH;
        else
            process.env.CLAWDFUN_CONFIG_PATH = previousConfigPath;
        if (previousSkipBrowserControl === undefined)
            delete process.env.CLAWDFUN_SKIP_BROWSER_CONTROL_SERVER;
        else
            process.env.CLAWDFUN_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
        if (previousSkipGmailWatcher === undefined)
            delete process.env.CLAWDFUN_SKIP_GMAIL_WATCHER;
        else
            process.env.CLAWDFUN_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
        if (previousSkipCanvasHost === undefined)
            delete process.env.CLAWDFUN_SKIP_CANVAS_HOST;
        else
            process.env.CLAWDFUN_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
...
export async function startServerWithClient(token, opts) {
    ...
    const prev = process.env.CLAWDFUN_GATEWAY_TOKEN;
    if (token === undefined) {
        delete process.env.CLAWDFUN_GATEWAY_TOKEN;
    }
    else {
        process.env.CLAWDFUN_GATEWAY_TOKEN = token;
    }
    let server = null;
