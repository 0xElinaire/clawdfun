...
const SAFE_DAEMON_ENV_KEYS = [
    "CLAWDFUN_PROFILE",
    "CLAWDFUN_STATE_DIR",
    "CLAWDFUN_CONFIG_PATH",
    "CLAWDFUN_GATEWAY_PORT",
    "CLAWDFUN_NIX_MODE",
];
...
export function renderRuntimeHints(runtime, env = process.env) {
    ...
    if (runtime.missingUnit) {
        hints.push(`Service not installed. Run: ${formatCliCommand("clawdfun gateway install", env)}`);
        if (fileLog)
    ...
    if (runtime.status === "stopped") {
        ...
        if (process.platform === "darwin") {
            ...
            const task = resolveGatewayWindowsTaskName(env.CLAWDFUN_PROFILE);
...
export function renderGatewayServiceStartHints(env = process.env) {
    const base = [
        formatCliCommand("clawdfun gateway install", env),
        formatCliCommand("clawdfun gateway", env),
    ];
