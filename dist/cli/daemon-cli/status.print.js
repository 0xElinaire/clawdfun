...
export function printDaemonStatus(status, opts) {
    ...
    if (service.configAudit?.issues.length) {
        ...
        defaultRuntime.error(warnText(`Recommendation: run "${formatCliCommand("clawdfun doctor")}" (or "${formatCliCommand("clawdfun doctor --repair")}").`));
    }
    if (status.config) {
        ...
        if (status.config.mismatch) {
            ...
            defaultRuntime.error(errorText(`Fix: rerun \`${formatCliCommand("clawdfun gateway install --force")}\` from the same --profile / CLAWDFUN_STATE_DIR you expect.`));
        }
        spacer();
    }
    if (status.gateway) {
    ...
    if (service.runtime?.cachedLabel) {
        ...
        const labelValue = resolveGatewayLaunchAgentLabel(env.CLAWDFUN_PROFILE);
        ...
        defaultRuntime.error(errorText(`Then reinstall: ${formatCliCommand("clawdfun gateway install")}`));
        spacer();
    }
    ...
    if (service.loaded &&
        ...
        if (process.platform === "linux") {
            const env = (service.command?.environment ?? process.env);
            const unit = resolveGatewaySystemdServiceName(env.CLAWDFUN_PROFILE);
    ...
    if (legacyServices.length > 0) {
        ...
        defaultRuntime.error(errorText(`Cleanup: ${formatCliCommand("clawdfun doctor")}`));
        spacer();
    }
    ...
    defaultRuntime.log(`${label("Troubles:")} run ${formatCliCommand("clawdfun status")}`);
