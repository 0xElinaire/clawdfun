 ...
 */
function formatHookLine(hook, verbose = false) {
    ...
    const desc = chalk.gray(hook.description.length > 50 ? `${hook.description.slice(0, 47)}...` : hook.description);
    if (verbose) {
        const missing = [];
...
export function formatHooksList(report, opts) {
    ...
    if (hooks.length === 0) {
        const message = opts.eligible
            ? "No eligible hooks found. Run `clawdfun hooks list` to see all hooks."
            : "No hooks found.";
        return message;
    }
    ...
    const lines = [];
    lines.push(chalk.bold.cyan("Internal Hooks") + chalk.gray(` (${eligible.length}/${hooks.length} ready)`));
    lines.push("");
...
export function formatHookInfo(report, hookName, opts) {
    ...
    if (!hook) {
        ...
        return `Hook "${hookName}" not found. Run \`clawdfun hooks list\` to see available hooks.`;
    }
    if (opts.json) {
...
export function registerInternalHooksSubcommands(hooksCommand) {
