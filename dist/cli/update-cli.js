...
import { resolveClawdfunPackageRoot } from "../infra/clawdfun-root.js";
...
const STEP_LABELS = {
    ...
    "clawdfun doctor": "Running doctor checks",
...
const CLAWDFUN_REPO_URL = "https://github.com/clawdfun/clawdfun.git";
const DEFAULT_GIT_DIR = path.join(os.homedir(), "clawdfun");
function normalizeTag(value) {
    ...
    if (!trimmed)
        return null;
    return trimmed.startsWith("clawdfun@") ? trimmed.slice("clawdfun@".length) : trimmed;
}
...
async function isClawdfunPackage(root) {
    try {
        const raw = await fs.readFile(path.join(root, "package.json"), "utf-8");
        const parsed = JSON.parse(raw);
        return parsed?.name === "clawdfun";
    }
    catch {
        return false;
    }
...
function resolveGitInstallDir() {
    const override = process.env.CLAWDFUN_GIT_DIR?.trim();
    if (override)
...
async function ensureGitCheckout(params) {
    ...
    if (!dirExists) {
        return await runUpdateStep({
            name: "git clone",
            argv: ["git", "clone", CLAWDFUN_REPO_URL, params.dir],
            timeoutMs: params.timeoutMs,
    ...
    if (!(await isGitCheckout(params.dir))) {
        ...
        if (!empty) {
            throw new Error(`CLAWDFUN_GIT_DIR points at a non-git directory: ${params.dir}. Set CLAWDFUN_GIT_DIR to an empty folder or a clawdfun checkout.`);
        }
        return await runUpdateStep({
            name: "git clone",
            argv: ["git", "clone", CLAWDFUN_REPO_URL, params.dir],
    ...
    if (!(await isClawdfunPackage(params.dir))) {
        throw new Error(`CLAWDFUN_GIT_DIR does not look like a clawdfun checkout: ${params.dir}.`);
    }
    return null;
}
...
export async function updateStatusCommand(opts) {
    ...
    const root = (await resolveClawdfunPackageRoot({
    ...
    defaultRuntime.log(theme.heading("Clawdfun update status"));
    defaultRuntime.log("");
...
export async function updateCommand(opts) {
    ...
    if (!opts.json) {
        defaultRuntime.log(theme.heading("Updating Clawdfun..."));
        defaultRuntime.log("");
    }
    ...
    if (switchToPackage) {
        ...
        const updateStep = await runUpdateStep({
            name: "global update",
            argv: globalInstallArgs(manager, `clawdfun@${tag}`),
            timeoutMs: timeoutMs ?? 20 * 60_000,
            progress,
        });
    ...
    if (result.status === "skipped") {
        ...
        if (result.reason === "not-git-install") {
            defaultRuntime.log(theme.warn(`Skipped: this Clawdfun install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${formatCliCommand("clawdfun doctor")}\` and \`${formatCliCommand("clawdfun gateway restart")}\`.`));
            defaultRuntime.log(theme.muted("Examples: `npm i -g clawdfun@latest` or `pnpm add -g clawdfun@latest`"));
        }
        defaultRuntime.exit(0);
        return;
    }
    ...
    if (shouldRestart) {
        ...
        try {
            ...
            if (!opts.json && restarted) {
                ...
                defaultRuntime.log("");
                process.env.CLAWDFUN_UPDATE_IN_PROGRESS = "1";
                try {
                    ...
                    delete process.env.CLAWDFUN_UPDATE_IN_PROGRESS;
                }
            }
        }
        catch (err) {
            if (!opts.json) {
                ...
                defaultRuntime.log(theme.muted(`You may need to restart the service manually: ${formatCliCommand("clawdfun gateway restart")}`));
            }
        }
    }
    else if (!opts.json) {
        ...
        if (result.mode === "npm" || result.mode === "pnpm") {
            defaultRuntime.log(theme.muted(`Tip: Run \`${formatCliCommand("clawdfun doctor")}\`, then \`${formatCliCommand("clawdfun gateway restart")}\` to apply updates to a running gateway.`));
        }
        else {
            defaultRuntime.log(theme.muted(`Tip: Run \`${formatCliCommand("clawdfun gateway restart")}\` to apply updates to a running gateway.`));
        }
    }
    if (!opts.json) {
...
export async function updateWizardCommand(opts = {}) {
    if (!process.stdin.isTTY) {
        defaultRuntime.error("Update wizard requires a TTY. Use `clawdfun update --channel <stable|beta|dev>` instead.");
        defaultRuntime.exit(1);
        return;
    }
    const timeoutMs = opts.timeout ? Number.parseInt(opts.timeout, 10) * 1000 : undefined;
    ...
    if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
        ...
        if (!hasGit) {
            ...
            if (dirExists) {
                ...
                if (!empty) {
                    defaultRuntime.error(`CLAWDFUN_GIT_DIR points at a non-git directory: ${gitDir}. Set CLAWDFUN_GIT_DIR to an empty folder or a clawdfun checkout.`);
                    defaultRuntime.exit(1);
                    return;
                }
            }
            const ok = await confirm({
                message: stylePromptMessage(`Create a git checkout at ${gitDir}? (override via CLAWDFUN_GIT_DIR)`),
                initialValue: true,
            });
            if (isCancel(ok) || ok === false) {
                defaultRuntime.log(theme.muted("Update cancelled."));
...
export function registerUpdateCli(program) {
    const update = program
        .command("update")
        .description("Update Clawdfun to the latest version")
        ...
        .addHelpText("after", () => {
        const examples = [
            ["clawdfun update", "Update a source checkout (git)"],
            ["clawdfun update --channel beta", "Switch to beta channel (git + npm)"],
            ["clawdfun update --channel dev", "Switch to dev channel (git + npm)"],
            ["clawdfun update --tag beta", "One-off update to a dist-tag or version"],
            ["clawdfun update --no-restart", "Update without restarting the service"],
            ["clawdfun update --json", "Output result as JSON"],
            ["clawdfun update --yes", "Non-interactive (accept downgrade prompts)"],
            ["clawdfun update wizard", "Interactive update wizard"],
            ["clawdfun --update", "Shorthand for clawdfun update"],
        ];
...
${theme.heading("Switch channels:")}
  ...
  - Run clawdfun update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.clawd.bot/cli/update")}`;
    ...
    update
        ...
        .addHelpText("after", () => `\n${theme.heading("Examples:")}\n${formatHelpExamples([
        ["clawdfun update status", "Show channel + version status."],
        ["clawdfun update status --json", "JSON output."],
        ["clawdfun update status --timeout 10", "Custom timeout."],
