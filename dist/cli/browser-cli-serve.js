...
export function registerBrowserServeCommands(browser, _parentOpts) {
    browser
        .command("serve")
        ...
        .option("--token <token>", "Require Authorization: Bearer <token> (required when binding non-loopback)")
        .action(async (opts) => {
        const cfg = loadConfig();
        const resolved = resolveBrowserConfig(cfg.browser);
        if (!resolved.enabled) {
            defaultRuntime.error(danger("Browser control is disabled. Set browser.enabled=true and try again."));
        ...
        if (!isLoopbackBindHost(host) && !authToken) {
            defaultRuntime.error(danger(`Refusing to bind browser control on ${host} without --token (or CLAWDFUN_BROWSER_CONTROL_TOKEN, or browser.controlToken).`));
            defaultRuntime.exit(1);
        }
        ...
        for (const name of Object.keys(resolved.profiles)) {
            ...
            await ensureChromeExtensionRelayServer({ cdpUrl: profile.cdpUrl }).catch((err) => {
                defaultRuntime.error(danger(`Chrome extension relay init failed for profile "${name}": ${String(err)}`));
            });
        }
        defaultRuntime.log(info([
            ...
            "",
            "Paste on the Gateway (clawdfun.json):",
            JSON.stringify({
                browser: {
                    enabled: true,
            ...
            ...(authToken
                ? [
                    ...
                    `export CLAWDFUN_BROWSER_CONTROL_TOKEN=${JSON.stringify(authToken)}`,
                ]
                : []),
        ].join("\n")));
