...
function renderNodeServiceStartHints() {
    const base = [formatCliCommand("clawdfun node install"), formatCliCommand("clawdfun node start")];
...
export async function runNodeDaemonInstall(opts) {
    ...
    if (loaded && !opts.force) {
        ...
        if (!json) {
            ...
            defaultRuntime.log(`Reinstall with: ${formatCliCommand("clawdfun node install --force")}`);
        }
        return;
    }
...
export async function runNodeDaemonStatus(opts = {}) {
    ...
    const hintEnv = {
        ...baseEnv,
        CLAWDFUN_LOG_PREFIX: baseEnv.CLAWDFUN_LOG_PREFIX ?? "node",
    };
