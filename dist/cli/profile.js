...
function resolveProfileStateDir(profile, homedir) {
    ...
    return path.join(homedir(), `.clawdfun${suffix}`);
}
export function applyCliProfileEnv(params) {
    ...
    env.CLAWDFUN_PROFILE = profile;
    const stateDir = env.CLAWDFUN_STATE_DIR?.trim() || resolveProfileStateDir(profile, homedir);
    if (!env.CLAWDFUN_STATE_DIR?.trim())
        env.CLAWDFUN_STATE_DIR = stateDir;
    if (!env.CLAWDFUN_CONFIG_PATH?.trim()) {
        env.CLAWDFUN_CONFIG_PATH = path.join(stateDir, "clawdfun.json");
    }
    if (profile === "dev" && !env.CLAWDFUN_GATEWAY_PORT?.trim()) {
        env.CLAWDFUN_GATEWAY_PORT = "19001";
    }
}
