...
import { STATE_DIR_CLAWDFUN } from "../config/paths.js";
...
function installedExtensionRootDir() {
    return path.join(STATE_DIR_CLAWDFUN, "browser", "chrome-extension");
}
...
export async function installChromeExtension(opts) {
    ...
    if (!hasManifest(src)) {
        throw new Error("Bundled Chrome extension is missing. Reinstall Clawdfun and try again.");
    }
    const stateDir = opts?.stateDir ?? STATE_DIR_CLAWDFUN;
...
export function registerBrowserExtensionCommands(browser, parentOpts) {
    ...
    ext
        ...
        defaultRuntime.error(info([
            ...
            `- Pin “Clawdfun Browser Relay”, then click it on the tab (badge shows ON)`,
            "",
    ...
    ext
        ...
        if (!hasManifest(dir)) {
            defaultRuntime.error(danger([
                `Chrome extension is not installed. Run: "${formatCliCommand("clawdfun browser extension install")}"`,
