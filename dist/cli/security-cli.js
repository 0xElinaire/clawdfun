...
export function registerSecurityCli(program) {
    ...
    security
        ...
        const lines = [];
        lines.push(heading("Clawdfun security audit"));
        lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
        lines.push(muted(`Run deeper: ${formatCliCommand("clawdfun security audit --deep")}`));
        if (opts.fix) {
            lines.push(muted(`Fix: ${formatCliCommand("clawdfun security audit --fix")}`));
            if (!fixResult) {
