...
import { runDaemonInstall, runDaemonRestart, runDaemonStart, runDaemonStatus, runDaemonStop, runDaemonUninstall, } from "./daemon-cli/runners.js";
...
export function registerServiceCli(program) {
    const gatewayExamples = [
        ["clawdfun service gateway status", "Show gateway service status + probe."],
        [
            "clawdfun service gateway install --port 18789 --token <token>",
            "Install the Gateway service on port 18789.",
        ],
        ["clawdfun service gateway restart", "Restart the Gateway service."],
    ];
    const nodeExamples = [
        ["clawdfun service node status", "Show node host service status."],
        [
            "clawdfun service node install --host gateway.local --port 18789 --tls",
            "Install the node host service with TLS.",
        ],
        ["clawdfun service node restart", "Restart the node host service."],
    ];
