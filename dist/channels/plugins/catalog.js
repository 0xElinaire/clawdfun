...
import { discoverClawdfunPlugins } from "../../plugins/discovery.js";
...
const ENV_CATALOG_PATHS = ["CLAWDFUN_PLUGIN_CATALOG_PATHS", "CLAWDFUN_MPM_CATALOG_PATHS"];
...
function buildCatalogEntry(candidate) {
    const manifest = candidate.packageClawdfun;
    if (!manifest?.channel)
        return null;
...
function buildExternalCatalogEntry(entry) {
    return buildCatalogEntry({
        ...
        packageClawdfun: entry.clawdfun,
    });
}
...
export function listChannelPluginCatalogEntries(options = {}) {
    const discovery = discoverClawdfunPlugins({ workspaceDir: options.workspaceDir });
