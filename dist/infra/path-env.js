...
function candidateBinDirs(opts) {
    ...
    const candidates = [];
    // Bundled macOS app: `clawdfun` lives next to the executable (process.execPath).
    try {
        const execDir = path.dirname(execPath);
        const siblingClawdfun = path.join(execDir, "clawdfun");
        if (isExecutable(siblingClawdfun))
            candidates.push(execDir);
    }
    catch {
        // ignore
    }
    // Project-local installs (best effort): if a `node_modules/.bin/clawdfun` exists near cwd,
    // include it. This helps when running under launchd or other minimal PATH environments.
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "clawdfun")))
...
/**
 * Best-effort PATH bootstrap so skills that require the `clawdfun` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureClawdfunCliOnPath(opts = {}) {
    if (isTruthyEnvValue(process.env.CLAWDFUN_PATH_BOOTSTRAPPED))
        return;
    process.env.CLAWDFUN_PATH_BOOTSTRAPPED = "1";
