...
export async function resolveGlobalPackageRoot(manager, runCommand, timeoutMs) {
    ...
    if (!root)
        return null;
    return path.join(root, "clawdfun");
}
...
export async function detectGlobalInstallManagerByPresence(runCommand, timeoutMs) {
    for (const manager of ["npm", "pnpm"]) {
        ...
        if (!root)
            continue;
        if (await pathExists(path.join(root, "clawdfun")))
            return manager;
    }
    const bunRoot = resolveBunGlobalRoot();
    if (await pathExists(path.join(bunRoot, "clawdfun")))
        return "bun";
    return null;
}
