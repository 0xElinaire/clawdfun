...
export async function runGatewayUpdate(opts = {}) {
    ...
    if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
        ...
        const statusCheck = await runStep(step("clean check", ["git", "-C", gitRoot, "status", "--porcelain"], gitRoot));
        ...
        if (channel === "dev") {
            ...
            const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "clawdfun-update-preflight-"));
        ...
        const doctorStep = await runStep(step("clawdfun doctor", managerScriptArgs(manager, "clawdfun", ["doctor", "--non-interactive"]), gitRoot, { CLAWDFUN_UPDATE_IN_PROGRESS: "1" }));
    ...
    if (globalManager) {
        const spec = `clawdfun@${normalizeTag(opts.tag)}`;
