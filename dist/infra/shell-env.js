...
export function loadShellEnvFallback(opts) {
    ...
    try {
        ...
        logger.warn(`[clawdfun] shell env fallback failed: ${msg}`);
        lastAppliedKeys = [];
...
export function shouldEnableShellEnvFallback(env) {
    return isTruthyEnvValue(env.CLAWDFUN_LOAD_SHELL_ENV);
}
export function shouldDeferShellEnvFallback(env) {
    return isTruthyEnvValue(env.CLAWDFUN_DEFER_SHELL_ENV_FALLBACK);
}
export function resolveShellEnvFallbackTimeoutMs(env) {
    const raw = env.CLAWDFUN_SHELL_ENV_TIMEOUT_MS?.trim();
    if (!raw)
