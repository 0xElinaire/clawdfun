...
export const WIDE_AREA_DISCOVERY_DOMAIN = "clawdfun.internal.";
export const WIDE_AREA_ZONE_FILENAME = "clawdfun.internal.db";
...
function extractContentHash(zoneText) {
    const match = zoneText.match(/^\s*;\s*clawdfun-content-hash:\s*(\S+)\s*$/m);
    return match?.[1] ?? null;
}
...
function renderZone(opts) {
    const hostname = os.hostname().split(".")[0] ?? "clawdfun";
    const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "clawdfun");
    const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "clawdfun-gw");
    const txt = [
    ...
    records.push(`_clawdfun-gw._tcp IN PTR ${instanceLabel}._clawdfun-gw._tcp`);
    records.push(`${instanceLabel}._clawdfun-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
    records.push(`${instanceLabel}._clawdfun-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
    ...
    return `; clawdfun-content-hash: ${contentHash}\n${contentBody}`;
}
