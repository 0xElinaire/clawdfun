...
function buildMessagingSection(params) {
    ...
    return [
        ...
        "- Never use exec/curl for provider messaging; Clawdfun handles all routing internally.",
...
function buildDocsSection(params) {
    ...
    return [
        "## Documentation",
        `Clawdfun docs: ${docsPath}`,
        "Mirror: https://docs.clawd.bot",
        "Source: https://github.com/clawdfun/clawdfun",
        "Community: https://discord.com/invite/clawd",
        "Find new skills: https://clawdhub.com",
        "For Clawdfun behavior, commands, config, or architecture: consult local docs first.",
        "When diagnosing issues, run `clawdfun status` yourself when possible; only ask the user if you lack access (e.g., sandboxed).",
        "",
    ];
}
export function buildAgentSystemPrompt(params) {
    const coreToolSummaries = {
        ...
        gateway: "Restart, apply config, or run updates on the running Clawdfun process",
    ...
    if (promptMode === "none") {
        return "You are a personal assistant running inside Clawdfun.";
    }
    const lines = [
        "You are a personal assistant running inside Clawdfun.",
        "",
        "## Tooling",
        ...
        "",
        "## Clawdfun CLI Quick Reference",
        "Clawdfun is controlled via subcommands. Do not invent commands.",
        "To manage the Gateway daemon service (start/stop/restart):",
        "- clawdfun gateway status",
        "- clawdfun gateway start",
        "- clawdfun gateway stop",
        "- clawdfun gateway restart",
        "If unsure, ask the user to run `clawdfun help` (or `clawdfun gateway --help`) and paste the output.",
        "",
        ...skillsSection,
        ...memorySection,
        // Skip self-update for subagent/none modes
        hasGateway && !isMinimal ? "## Clawdfun Self-Update" : "",
        hasGateway && !isMinimal
            ? [
                "Get Updates (self-update) is ONLY allowed when the user explicitly asks for it.",
                "Do not run config.apply or update.run unless the user explicitly requests an update or config change; if it's not explicit, ask first.",
                "Actions: config.get, config.schema, config.apply (validate + write full config, then restart), update.run (update deps or git, then restart).",
                "After restart, Clawdfun pings the last active session automatically.",
            ].join("\n")
            : "",
        hasGateway && !isMinimal ? "" : "",
        "",
        ...
        "These user-editable files are loaded by Clawdfun and included below in Project Context.",
        "",
