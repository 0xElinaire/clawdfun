...
export function buildSandboxCreateArgs(params) {
    ...
    args.push("--label", "clawdfun.sandbox=1");
    args.push("--label", `clawdfun.sessionKey=${params.scopeKey}`);
    args.push("--label", `clawdfun.createdAtMs=${createdAtMs}`);
    if (params.configHash) {
        args.push("--label", `clawdfun.configHash=${params.configHash}`);
    }
...
async function readContainerConfigHash(containerName) {
    const result = await execDocker(["inspect", "-f", '{{ index .Config.Labels "clawdfun.configHash" }}', containerName], { allowFailure: true });
    if (result.code !== 0)
        return null;
...
function formatSandboxRecreateHint(params) {
    if (params.scope === "session") {
        return formatCliCommand(`clawdfun sandbox recreate --session ${params.sessionKey}`);
    }
    if (params.scope === "agent") {
        const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
        return formatCliCommand(`clawdfun sandbox recreate --agent ${agentId}`);
    }
    return formatCliCommand("clawdfun sandbox recreate --all");
}
