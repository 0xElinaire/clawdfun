...
import { compareClawdfunVersions } from "./version.js";
...
const SHELL_ENV_EXPECTED_KEYS = [
    ...
    "SLACK_BOT_TOKEN",
    "SLACK_APP_TOKEN",
    "CLAWDFUN_GATEWAY_TOKEN",
    "CLAWDFUN_GATEWAY_PASSWORD",
];
...
function warnIfConfigFromFuture(cfg, logger) {
    ...
    if (!touched)
        return;
    const cmp = compareClawdfunVersions(VERSION, touched);
    if (cmp === null)
        return;
    if (cmp < 0) {
        logger.warn(`Config was last written by a newer Clawdfun (${touched}); current version is ${VERSION}.`);
    }
}
...
// module scope. `CLAWDFUN_CONFIG_PATH` (and friends) are expected to work even
// when set after the module has been imported (tests, one-off scripts, etc.).
const DEFAULT_CONFIG_CACHE_MS = 200;
let configCache = null;
function resolveConfigCacheMs(env) {
    const raw = env.CLAWDFUN_CONFIG_CACHE_MS?.trim();
    if (raw === "" || raw === "0")
        return 0;
    if (!raw)
...
function shouldUseConfigCache(env) {
    if (env.CLAWDFUN_DISABLE_CONFIG_CACHE?.trim())
        return false;
