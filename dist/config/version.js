...
export function parseClawdfunVersion(raw) {
    if (!raw)
        return null;
...
export function compareClawdfunVersions(a, b) {
    const parsedA = parseClawdfunVersion(a);
    const parsedB = parseClawdfunVersion(b);
    if (!parsedA || !parsedB)
        return null;
