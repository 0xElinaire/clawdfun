...
const BUNDLED_VERSION = (typeof __CLAWDFUN_VERSION__ === "string" && __CLAWDFUN_VERSION__) ||
...
async function main() {
    if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
        // Match `clawdfun --version` behavior for Swift env/version checks.
    ...
    const portRaw = argValue(args, "--port") ??
        process.env.CLAWDFUN_GATEWAY_PORT ??
    ...
    const bindRaw = argValue(args, "--bind") ??
        process.env.CLAWDFUN_GATEWAY_BIND ??
        cfg.gateway?.bind ??
        "loopback";
...
void main().catch((err) => {
    console.error("[clawdfun] Gateway daemon failed:", err instanceof Error ? (err.stack ?? err.message) : err);
    process.exit(1);
});
