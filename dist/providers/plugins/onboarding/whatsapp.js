...
async function promptWhatsAppAllowFrom(cfg, _runtime, prompter, options) {
    ...
    if (options?.forceAllowlist) {
        await prompter.note("We need the sender/owner number so Clawdfun can allowlist you.", "WhatsApp number");
    ...
    const phoneMode = (await prompter.select({
        ...
        options: [
            ...
            { value: "separate", label: "Separate phone just for Clawdfun" },
        ],
    }));
...
export const whatsappOnboardingAdapter = {
    ...
    configure: async ({ cfg, runtime, prompter, options, accountOverrides, shouldPromptAccountIds, forceAllowFrom, }) => {
        ...
        if (wantsLink) {
            ...
            await prompter.note("Run `clawdfun providers login` later to link WhatsApp.", "WhatsApp");
        }
