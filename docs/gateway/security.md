...
## Quick check: `clawdfun security audit`

Run this regularly (especially after changing config or exposing network surfaces):

```bash
clawdfun security audit
clawdfun security audit --deep
clawdfun security audit --fix
```
...
- Tighten local perms (`~/.clawdfun` → `700`, config file → `600`, plus common state files like `credentials/*.json`, `agents/*/agent/auth-profiles.json`, and `agents/*/sessions/sessions.json`).

Running an AI agent with shell access on your machine is... *spicy*. Here’s how to not get pwned.

Clawdfun is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
If you run `--deep`, Clawdfun also attempts a best-effort live Gateway probe.
...
`clawdfun security audit` warns when this setting is enabled.

## Local session logs live on disk

Clawdfun stores session transcripts on disk under `~/.clawdfun/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.clawdfun` (see the audit section below). If you need
...
Clawdfun can refresh the skills list mid-session:
...
- If you install plugins from npm (`clawdfun plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.clawdfun/extensions/<pluginId>/` (or `$CLAWDBOT_STATE_DIR/extensions/<pluginId>/`).
  - Clawdfun uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
clawdfun pairing approve <channel> <code>
```
...
By default, Clawdfun routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
Clawdfun has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.dm.allowFrom` / `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/.clawdfun/credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
4. **Re-run audit**
   - `clawdfun security audit --deep` and confirm the report is clean.
...
- `~/.clawdfun/clawdfun.json`: `600` (user read/write only)
- `~/.clawdfun`: `700` (user only)

`clawdfun doctor` can warn and offer to tighten these permissions.
...
Doctor can generate one for you: `clawdfun doctor --generate-gateway-token`.
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), Clawdfun
...
- Clawdfun will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.clawdfun/` (or `$CLAWDBOT_STATE_DIR/`) may contain secrets or private data:

- `clawdfun.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `clawdfun status --all` (pasteable, secrets redacted) over raw logs.
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.clawdfun/sandboxes`
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `clawdfun gateway` process.
...
1. Check Gateway logs: `/tmp/clawdfun/clawdfun-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.clawdfun/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).

### Collect for a report

- Timestamp, gateway host OS + Clawdfun version
...
Found a vulnerability in Clawdfun? Please report responsibly:
