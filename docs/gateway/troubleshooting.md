---
summary: "Quick troubleshooting guide for common Clawdfun failures"
read_when:
  - Investigating runtime issues or failures
---
# Troubleshooting 🔧

When Clawdfun misbehaves, here's how to fix it.
...
|---|---|---|
| `clawdfun status` | Local summary: OS + update, gateway reachability/mode, service, agents/sessions, provider config state | First check, quick overview |
| `clawdfun status --all` | Full local diagnosis (read-only, pasteable, safe-ish) incl. log tail | When you need to share a debug report |
| `clawdfun status --deep` | Runs gateway health checks (incl. provider probes; requires reachable gateway) | When “configured” doesn’t mean “working” |
| `clawdfun gateway probe` | Gateway discovery + reachability (local + remote targets) | When you suspect you’re probing the wrong gateway |
| `clawdfun channels status --probe` | Asks the running gateway for channel status (and optionally probes) | When gateway is reachable but channels misbehave |
| `clawdfun gateway status` | Supervisor state (launchd/systemd/schtasks), runtime PID/exit, last gateway error | When the service “looks loaded” but nothing runs |
| `clawdfun logs --follow` | Live logs (best signal for runtime issues) | When you need the actual failure reason |

**Sharing output:** prefer `clawdfun status --all` (it redacts tokens). If you paste `clawdfun status`, consider setting `CLAWDBOT_SHOW_SECRETS=0` first (token previews).
...
- Or paste a setup-token on the **gateway host**:
  ```bash
  clawdfun models auth setup-token --provider anthropic
  ```
...
```bash
clawdfun models auth paste-token --provider anthropic
clawdfun models status
```

**If you want to keep OAuth reuse:**
log in with Claude Code CLI on the gateway host, then run `clawdfun models status`
to sync the refreshed token into Clawdfun’s auth store.
...
**Logs:**
- Preferred: `clawdfun logs --follow`
- File logs (always): `/tmp/clawdfun/clawdfun-YYYY-MM-DD.log` (or your configured `logging.file`)
- macOS LaunchAgent (if installed): `$CLAWDBOT_STATE_DIR/logs/gateway.log` and `gateway.err.log`
- Linux systemd (if installed): `journalctl --user -u clawdfun-gateway[-<profile>].service -n 200 --no-pager`
...
- Set a remote URL and keep `gateway.mode=remote`:
  ```bash
  clawdfun config set gateway.mode remote
  clawdfun config set gateway.remote.url "wss://gateway.example.com"
  ```

**Ad-hoc/dev only:** pass `--allow-unconfigured` to start the gateway without
`gateway.mode=local`.

**No config file yet?** Run `clawdfun setup` to create a starter config, then rerun
the gateway.
...
variables like `DISPLAY` should live in `~/.clawdfun/.env` (loaded early by the
gateway).
...
service was installed with Bun or a version-managed Node path, run `clawdfun doctor`
...
- then run `clawdfun sandbox recreate --agent <id>` (or `--all`)
...
**Check:**
- `gateway.mode` must be `local` for `clawdfun gateway` and the service.
- If you set `gateway.mode=remote`, the **CLI defaults** to a remote URL. The service can still be running locally, but your CLI may be probing the wrong place. Use `clawdfun gateway status` to see the service’s resolved port + probe target (or pass `--url`).
- `clawdfun gateway status` and `clawdfun doctor` surface the **last gateway error** from logs when the service looks running but the port is closed.
...
**If `clawdfun gateway status` shows a config mismatch**
...
- Fix: rerun `clawdfun gateway install --force` from the same `--profile` / `CLAWDBOT_STATE_DIR` you want the service to use.

**If `clawdfun gateway status` reports service config issues**
- The supervisor config (launchd/systemd/schtasks) is missing current defaults.
- Fix: run `clawdfun doctor` to update it (or `clawdfun gateway install --force` for a full rewrite).
...
**If `clawdfun gateway status` says `bind=tailnet` but no tailnet interface was found**
...
If you upgraded from older installs, you might still have `~/clawdfun` on disk.
...
Symptoms: `pwd` or file tools show `~/.clawdfun/sandboxes/...` even though you
...
Clawdfun intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `clawdfun models list` or
  `clawdfun models scan` and choose a supported one.
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${CLAWDBOT_CONFIG_PATH:-$HOME/.clawdfun/clawdfun.json}"
```
...
tail -f "$(ls -t /tmp/clawdfun/clawdfun-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
clawdfun logs --follow | grep "pairing request"
```
...
```bash
ls -la ~/.clawdfun/agents/<agentId>/sessions/
```
...
clawdfun logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
clawdfun gateway --verbose
```

If you’re logged out / unlinked:

```bash
clawdfun channels logout
trash "${CLAWDBOT_STATE_DIR:-$HOME/.clawdfun}/credentials" # if logout can't cleanly remove everything
clawdfun channels login --verbose       # re-scan QR
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/clawdfun/clawdfun-*.log | head -1)" | tail -20
```

### High Memory Usage

Clawdfun keeps conversation history in memory.
...
Clawdfun now refuses to start when the config contains unknown keys, malformed values, or invalid types.
...
Notes:
- `clawdfun doctor` reports every invalid entry.
- `clawdfun doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `clawdfun logs`, `clawdfun health`, `clawdfun status`, `clawdfun gateway status`, and `clawdfun gateway probe` still run even if the config is invalid.

### “All models failed” — what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/clawdfun/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `clawdfun models status` (CLI).
...
5) Run `clawdfun channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. Clawdfun scrubs/normalizes tool
...
```bash
tccutil reset All com.clawdfun.mac.debug
```

**Fix 2: Force New Bundle ID**
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/clawdfun/clawdfun/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
# Or: launchctl bootout gui/$UID/com.clawdfun.gateway (replace with com.clawdfun.<profile> if needed)
```
...
Ensure the global `clawdfun` CLI is installed and matches the app version:
```bash
clawdfun --version
npm install -g clawdfun@<version>
```
...
#   ${CLAWDBOT_CONFIG_PATH:-$HOME/.clawdfun/clawdfun.json} -> { logging: { level: "trace" } }
#
# Then run verbose commands to mirror debug output to stdout:
clawdfun gateway --verbose
clawdfun channels login --verbose
```
...
| Log | Location |
|-----|----------|
| Gateway file logs (structured) | `/tmp/clawdfun/clawdfun-YYYY-MM-DD.log` (or `logging.file`) |
| Gateway service logs (supervisor) | macOS: `$CLAWDBOT_STATE_DIR/logs/gateway.log` + `gateway.err.log` (default: `~/.clawdfun/logs/...`; profiles use `~/.clawdfun-<profile>/logs/...`)<br />Linux: `journalctl --user -u clawdfun-gateway[-<profile>].service -n 200 --no-pager`<br />Windows: `schtasks /Query /TN "Clawdfun Gateway (<profile>)" /V /FO LIST` |
...
clawdfun gateway status --deep

# Is the gateway reachable?
clawdfun health --json
# If it fails, rerun with connection details:
clawdfun health --verbose
...
tail -20 /tmp/clawdfun/clawdfun-*.log
```
...
# clawdfun gateway uninstall

trash "${CLAWDBOT_STATE_DIR:-$HOME/.clawdfun}"
clawdfun channels login         # re-pair WhatsApp
clawdfun gateway restart           # or: clawdfun gateway
```
...
1. Check logs first: `/tmp/clawdfun/` (default: `clawdfun-YYYY-MM-DD.log`, or your configured `logging.file`)
