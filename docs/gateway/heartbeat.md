...
- During heartbeat runs, Clawdfun treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `clawdfun sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Clawdfun skips the heartbeat run entirely (no model call).
...
```bash
clawdfun system event --text "Check for urgent follow-ups" --mode now
```
