...
Clawdfun can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. Clawdfun requires a shared password.
...
(`tailscale-user-login`) without supplying a token/password. Clawdfun only
...
clawdfun gateway --tailscale funnel --auth password
```
...
- Set `gateway.tailscale.resetOnExit` if you want Clawdfun to undo `tailscale serve`
