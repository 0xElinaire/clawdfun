...
- Replaces the legacy `gateway` command. CLI entry point: `clawdfun gateway`.
- Runs until stopped; exits non-zero on fatal errors so the supervisor restarts it.

## How to run (local)
```bash
clawdfun gateway --port 18789
# for full debug/trace logs in stdio:
clawdfun gateway --port 18789 --verbose
# if the port is busy, terminate listeners then start:
clawdfun gateway --force
# dev loop (auto-reload on TS changes):
pnpm gateway:watch
```
- Config hot reload watches `~/.clawdfun/clawdfun.json` (or `CLAWDBOT_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__clawdfun__/canvas/` from `~/clawd/canvas`. Disable with `canvasHost.enabled=false` or `CLAWDBOT_SKIP_CANVAS_HOST=1`.
...
- macOS: `com.clawdfun.<profile>`
- Linux: `clawdfun-gateway-<profile>.service`
- Windows: `Clawdfun Gateway (<profile>)`

Install metadata is embedded in the service config:
- `CLAWDBOT_SERVICE_MARKER=clawdfun`
...
```bash
clawdfun --dev setup
clawdfun --dev gateway --allow-unconfigured
...
- `CLAWDBOT_STATE_DIR=~/.clawdfun-dev`
- `CLAWDBOT_CONFIG_PATH=~/.clawdfun-dev/clawdfun.json`
...
```bash
clawdfun --profile main gateway install
clawdfun --profile rescue gateway install
```

Example:
```bash
CLAWDBOT_CONFIG_PATH=~/.clawdfun/a.json CLAWDBOT_STATE_DIR=~/.clawdfun-a clawdfun gateway --port 19001
CLAWDBOT_CONFIG_PATH=~/.clawdfun/b.json CLAWDBOT_STATE_DIR=~/.clawdfun-b clawdfun gateway --port 19002
```
...
## Methods (initial set)
- `health` — full health snapshot (same shape as `clawdfun health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `clawdfun`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `clawdfun gateway install` writes `~/Library/LaunchAgents/com.clawdfun.gateway.plist`
    (or `com.clawdfun.<profile>.plist`).
  - `clawdfun doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are Clawdfun profile services.
  ...
  - Cleanup: `clawdfun gateway uninstall` (current service) and `clawdfun doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `clawdfun gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:
- Clawdfun.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `com.clawdfun.gateway` (or `com.clawdfun.<profile>`).
- To stop it cleanly, use `clawdfun gateway stop` (or `launchctl bootout gui/$UID/com.clawdfun.gateway`).
- To restart, use `clawdfun gateway restart` (or `launchctl kickstart -k gui/$UID/com.clawdfun.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `clawdfun gateway install` first.
  - Replace the label with `com.clawdfun.<profile>` when running a named profile.

## Supervision (systemd user unit)
Clawdfun installs a **systemd user service** by default on Linux/WSL2. We
...
`clawdfun gateway install` writes the user unit. `clawdfun doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/clawdfun-gateway[-<profile>].service`:
```
[Unit]
Description=Clawdfun Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/clawdfun gateway --port 18789
Restart=always
RestartSec=5
...
```
systemctl --user enable --now clawdfun-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/clawdfun-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:
```
sudo systemctl daemon-reload
sudo systemctl enable --now clawdfun-gateway[-<profile>].service
```
...
## CLI helpers
- `clawdfun gateway health|status` — request health/status over the Gateway WS.
- `clawdfun message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `clawdfun agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `clawdfun gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `clawdfun gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance
- Retire uses of `clawdfun gateway` and the legacy TCP control port.
