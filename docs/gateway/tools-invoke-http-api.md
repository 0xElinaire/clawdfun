...
Clawdfun’s Gateway exposes a simple HTTP endpoint for invoking a single tool directly. It is always enabled, but gated by Gateway auth and tool policy.
...
- `x-clawdfun-message-channel: <channel>` (example: `slack`, `telegram`)
- `x-clawdfun-account-id: <accountId>` (when multiple accounts exist)
