---
summary: "All configuration options for ~/.clawdfun/clawdfun.json with examples"
read_when:
  - Adding or modifying config fields
---
# Configuration 🔧

Clawdfun reads an optional **JSON5** config from `~/.clawdfun/clawdfun.json` (comments + trailing commas allowed).

If the file is missing, Clawdfun uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/clawd`). You usually only need a config to:
...
Clawdfun only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:
- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `clawdfun doctor`, `clawdfun logs`, `clawdfun health`, `clawdfun status`, `clawdfun service`, `clawdfun help`).
- Run `clawdfun doctor` to see the exact issues.
- Run `clawdfun doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `clawdfun config set`. Keep a backup of `~/.clawdfun/clawdfun.json`.
...
```bash
clawdfun gateway call config.get --params '{}' # capture payload.hash
clawdfun gateway call config.apply --params '{
...
```bash
clawdfun gateway call config.get --params '{}' # capture payload.hash
clawdfun gateway call config.patch --params '{
...
{ "$include": "/etc/clawdfun/base.json5" } // absolute
...
```json5
// ~/.clawdfun/clients/mueller/agents.json5
[
...
```json5
// ~/.clawdfun/clients/mueller/broadcast.json5
{
...
Clawdfun reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, Clawdfun runs your login shell and imports only the missing expected keys (never overrides).
...
Clawdfun stores **per-agent** auth profiles (OAuth + API keys) in:
- `<agentDir>/auth-profiles.json` (default: `~/.clawdfun/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:
- `~/.clawdfun/credentials/oauth.json` (or `$CLAWDBOT_STATE_DIR/credentials/oauth.json`)

The embedded Pi agent maintains a runtime cache at:
- `<agentDir>/auth.json` (managed automatically; don’t edit manually)

Legacy agent dir (pre multi-agent):
- `~/.clawdfun/agent/*` (migrated by `clawdfun doctor` into `~/.clawdfun/agents/<defaultAgentId>/agent/*`)
...
On first use, Clawdfun imports `oauth.json` entries into `auth-profiles.json`.

Clawdfun also auto-syncs OAuth tokens from external CLIs into `auth-profiles.json` (when present on the gateway host):
...
credential is a setup-token. Clawdfun auto-migrates older configs that used
`mode: "token"`.
...
If set, Clawdfun derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/clawdfun/clawdfun-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/clawdfun/clawdfun.log`.
...
```json5
{
  logging: {
    level: "info",
    file: "/tmp/clawdfun/clawdfun.log",
    consoleLevel: "info",
    consoleStyle: "pretty",
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.clawdfun/credentials/whatsapp/biz
          // authDir: "~/.clawdfun/credentials/whatsapp/biz",
        }
      }
    }
  }
...
- The legacy single-account Baileys auth dir is migrated by `clawdfun doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [
      { id: "main", groupChat: { mentionPatterns: ["@clawd", "clawdfun", "clawd"] } }
    ]
  }
}
```
...
- `agents.list[]`: per-agent overrides.
  ...
  - `agentDir`: default `~/.clawdfun/agents/<agentId>/agent`.
...
- `commands.config: true` enables `/config` (reads/writes `clawdfun.json`).
...
Clawdfun starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
...
Clawdfun starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `clawdfun plugins install @clawdfun/mattermost` (or `./extensions/mattermost` from a git checkout).
...
Clawdfun starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
Clawdfun spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, Clawdfun
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/clawdfun" } }
}
```
...
When a file exceeds this limit, Clawdfun logs a warning and injects a truncated
...
message envelopes). If unset, Clawdfun uses the host timezone at runtime.
...
`[clawdfun]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[clawdfun]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[clawdfun]"`, Clawdfun will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, Clawdfun generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.clawdfun/settings/tts.json",
      elevenlabs: {
...
Clawdfun also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, Clawdfun currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `clawdfun configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:clawdfun`: all built-in Clawdfun tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.clawdfun/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.clawdfun/sandboxes",
        docker: {
          image: "clawdfun-sandbox:bookworm-slim",
          containerPrefix: "clawdfun-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
          ...
          apparmorProfile: "clawdfun-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "clawdfun-sandbox-browser:bookworm-slim",
          containerPrefix: "clawdfun-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
Clawdfun uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.clawdfun/agents/<agentId>/agent/models.json` or by defining the same schema inside your
Clawdfun config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, Clawdfun writes/merges a `models.json` into
`~/.clawdfun/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. Clawdfun uses
...
- Shortcut: `clawdfun onboard --auth-choice opencode-zen`.
...
Shortcut: `clawdfun onboard --auth-choice zai-api-key`.
...
Notes:
- Set `MOONSHOT_API_KEY` in the environment or use `clawdfun onboard --auth-choice moonshot-api-key`.
...
Notes:
- Set `KIMICODE_API_KEY` in the environment or use `clawdfun onboard --auth-choice kimi-code-api-key`.
...
Notes:
- Set `SYNTHETIC_API_KEY` or use `clawdfun onboard --auth-choice synthetic-api-key`.
...
Notes:
- Set `MINIMAX_API_KEY` environment variable or use `clawdfun onboard --auth-choice minimax-api`.
...
- Override the agent config root with `CLAWDBOT_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.clawdfun/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.clawdfun/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.clawdfun/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, Clawdfun stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.clawdfun/skills` (workspace skills
...
from `~/.clawdfun/extensions`, `<workspace>/.clawdfun/extensions`, plus any
...
Clawdfun can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for clawd and expose a small loopback control server.
...
- Note: the control server is started by the running gateway (Clawdfun.app menubar, or `clawdfun gateway`).
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/clawdfun" }
...
- Examples: `"/ui"`, `"/clawdfun"`, `"/apps/clawdfun"`.
...
- When a connection comes from one of these IPs, Clawdfun uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:
- `clawdfun gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
...
macOS app behavior:
- Clawdfun.app watches `~/.clawdfun/clawdfun.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.clawdfun/clawdfun.json` (or `CLAWDBOT_CONFIG_PATH`) and applies changes automatically.
...
Files watched:
- `~/.clawdfun/clawdfun.json` (or `CLAWDBOT_CONFIG_PATH`)
...
- `clawdfun --dev …` → uses `~/.clawdfun-dev` + shifts ports from base `19001`
- `clawdfun --profile <name> …` → uses `~/.clawdfun-<name>` (port via config/env/flags)
...
Example:
```bash
CLAWDBOT_CONFIG_PATH=~/.clawdfun/a.json \
CLAWDBOT_STATE_DIR=~/.clawdfun-a \
clawdfun gateway --port 19001
```
...
Gmail helper config (used by `clawdfun webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "clawdfun@gmail.com",
...
Note: when `tailscale.mode` is on, Clawdfun defaults `serve.path` to `/` so
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__clawdfun/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__clawdfun__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.clawdfun/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.clawdfun/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.clawdfun/bridge/tls/bridge-key.pem"
    }
  }
}
```
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_clawdfun-bridge._tcp` under `~/.clawdfun/dns/` using the standard discovery domain `clawdfun.internal.`

To make iOS/Android discover across networks (Vienna ⇄ London), pair this with:
- a DNS server on the gateway host serving `clawdfun.internal.` (CoreDNS is recommended)
- Tailscale **split DNS** so clients resolve `clawdfun.internal` via that server
