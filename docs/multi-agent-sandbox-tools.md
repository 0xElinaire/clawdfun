...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `clawdfun sandbox explain`.
...
Legacy `agent.*` configs are migrated by `clawdfun doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ```exec
   docker ps --filter "label=clawdfun.sandbox=1"
   ```
...
4. **Monitor logs:**
   ```exec
   tail -f "${CLAWDBOT_STATE_DIR:-$HOME/.clawdfun}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
