...
This document explains how Clawdfun manages sessions end-to-end:
...
Clawdfun is designed around a single **Gateway process** that owns session state.
...
Clawdfun persists sessions in two layers:
...
- Transcripts: `~/.clawdfun/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

Clawdfun resolves these via `src/config/sessions.ts`.
...
Clawdfun intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (Clawdfun consumes the events, but Pi decides when to compact).
...
Clawdfun also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Clawdfun bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, Clawdfun leaves it alone.
...
- `clawdfun status` (CLI)
- `clawdfun sessions` / `sessions --json`
...
Clawdfun supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:
- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- Clawdfun strips/suppresses this in the delivery layer.

As of `2026.1.10`, Clawdfun also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
Clawdfun uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Clawdfun’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `clawdfun status`.
