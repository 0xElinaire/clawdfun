...
Clawdfun features that can generate provider usage or paid API calls.
...
- `clawdfun status --usage` and `clawdfun channels list` show provider **usage windows**
...
Clawdfun can pick up credentials from:
...
- `clawdfun status --usage`
- `clawdfun models status --json`
...
### 8) Model scan / probe
`clawdfun models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
