---
title: Fly.io
description: Deploy Clawdfun on Fly.io
---

# Fly.io Deployment

**Goal:** Clawdfun Gateway running on a [Fly.io](https://fly.io) machine with persistent storage, automatic HTTPS, and Discord/channel access.
...
fly apps create my-clawdfun

# Create a persistent volume (1GB is usually enough)
fly volumes create clawdfun_data --size 1 --region iad
```
...
**Note:** With `CLAWDBOT_STATE_DIR=/data`, the config path is `/data/clawdfun.json`.
...
Or visit `https://my-clawdfun.fly.dev/`
...
If using `--allow-unconfigured`, the gateway creates a minimal config. Your custom config at `/data/clawdfun.json` should be read on restart.

Verify the config exists:
```bash
fly ssh console --command "cat /data/clawdfun.json"
```
...
echo '{"your":"config"}' | fly ssh console -C "tee /data/clawdfun.json"

# Or use sftp
fly sftp shell
> put /local/path/config.json /data/clawdfun.json
```

**Note:** `fly sftp` may fail if the file already exists. Delete first:
```bash
fly ssh console --command "rm /data/clawdfun.json"
```
