---
summary: "Run Clawdfun Gateway on exe.dev (VM + HTTPS proxy) for remote access"
read_when:
...
Goal: Clawdfun Gateway running on an exe.dev VM, reachable from your laptop via:
...
1) Create VM → install Node 22 → install Clawdfun  
2) Run `clawdfun onboard --install-daemon`  
...
```bash
ssh exe.dev new --name=clawdfun
```

Then connect:

```bash
ssh clawdfun.exe.xyz
```

Tip: keep this VM **stateful**. Clawdfun stores state under `~/.clawdfun/` and `~/clawd/`.
...
```bash
ssh -N -L 18789:127.0.0.1:18789 clawdfun.exe.xyz
```
...
clawdfun gateway --bind lan --port 8080 --token "$CLAWDBOT_GATEWAY_TOKEN"
```

For service runs, persist it in `~/.clawdfun/clawdfun.json`:
...
```bash
ssh exe.dev share port clawdfun 8080
```

Open:
- `https://clawdfun.exe.xyz/`
...
On Linux, Clawdfun uses a systemd **user** service. After `--install-daemon`, verify:

```bash
systemctl --user status clawdfun-gateway[-<profile>].service
```
