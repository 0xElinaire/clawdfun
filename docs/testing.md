...
Clawdfun has three Vitest suites (unit/integration, e2e, live) and a small set of Docker runners.
...
### Layer 2: Gateway + dev agent smoke (what “@clawdfun” actually does)
...
```bash
clawdfun models auth paste-token --provider anthropic --profile-id anthropic:setup-token-test
...
- Gemini API vs Gemini CLI:
  - API: Clawdfun calls Google’s hosted Gemini API over HTTP (API key / profile auth); this is what most users mean by “Gemini”.
  - CLI: Clawdfun shells out to a local `gemini` binary; it has its own auth and can behave differently (streaming/tool support/version skew).
...
- OpenRouter: `openrouter/...` (hundreds of models; use `clawdfun models scan` to find tool+image capable candidates)
...
- If a live test says “no creds”, debug the same way you’d debug `clawdfun models list` / model selection.

- Profile store: `~/.clawdfun/credentials/` (preferred; what “profile keys” means in the tests)
- Config: `~/.clawdfun/clawdfun.json` (or `CLAWDBOT_CONFIG_PATH`)
...
- `CLAWDBOT_CONFIG_DIR=...` (default: `~/.clawdfun`) mounted to `/home/node/.clawdfun`
