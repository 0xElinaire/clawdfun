...
- `/usage cost` in chats: local cost summary aggregated from Clawdfun session logs.
- CLI: `clawdfun status --usage` prints a full per-provider breakdown.
- CLI: `clawdfun channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
