...
- **Session store** (chat history + routing state) under `~/.clawdfun/agents/<agentId>/sessions`.
...
available from `~/.clawdfun/skills`. See [Skills: per-agent vs shared](/tools/skills#per-agent-vs-shared-skills).
...
- State dir: `~/.clawdfun` (or `CLAWDBOT_STATE_DIR`)
- Workspace: `~/clawd` (or `~/clawd-<agentId>`)
- Agent dir: `~/.clawdfun/agents/<agentId>/agent` (or `agents.list[].agentDir`)
- Sessions: `~/.clawdfun/agents/<agentId>/sessions`

### Single-agent mode (default)

If you do nothing, Clawdfun runs a single agent:

- `agentId` defaults to **`main`**.
- Sessions are keyed as `agent:main:<mainKey>`.
- Workspace defaults to `~/clawd` (or `~/clawd-<profile>` when `CLAWDBOT_PROFILE` is set).
- State defaults to `~/.clawdfun/agents/main/agent`.
...
```js
{
    ...
    list: [
      {
        ...
        agentDir: "~/.clawdfun/agents/home/agent",
      },
      {
        id: "work",
        name: "Work",
        workspace: "~/clawd-work",
        agentDir: "~/.clawdfun/agents/work/agent",
      },
    ],
  },
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // Optional override. Default: ~/.clawdfun/credentials/whatsapp/personal
          // authDir: "~/.clawdfun/credentials/whatsapp/personal",
        },
        biz: {
