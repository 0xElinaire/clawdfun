---
summary: "Optional Docker-based setup and onboarding for Clawdfun"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run Clawdfun on a host without local installs.
- **No**: you’re running on your own machine and just want the fastest dev loop. Use the normal install flow instead.
- **Sandboxing note**: agent sandboxing uses Docker too, but it does **not** require the full gateway to run in Docker. See [Sandboxing](/gateway/sandboxing).

This guide covers:
- Containerized Gateway (full Clawdfun in Docker)
...
```bash
docker build -t clawdfun:local -f Dockerfile .
docker compose run --rm clawdfun-cli onboard
docker compose up -d clawdfun-gateway
```
...
`clawdfun-gateway` and `clawdfun-cli` by generating `docker-compose.extra.yml`.
...
```bash
export CLAWDBOT_HOME_VOLUME="clawdfun_home"
./docker-setup.sh
```
...
WhatsApp (QR):
```bash
docker compose run --rm clawdfun-cli channels login
```

Telegram (bot token):
```bash
docker compose run --rm clawdfun-cli channels add --channel telegram --token "<token>"
```

Discord (bot token):
```bash
docker compose run --rm clawdfun-cli channels add --channel discord --token "<token>"
```
...
```bash
docker compose exec clawdfun-gateway node dist/index.js health --token "$CLAWDBOT_GATEWAY_TOKEN"
```
...
- The gateway container is the source of truth for sessions (`~/.clawdfun/agents/<agentId>/sessions/`).
...
- Agent workspace access: `workspaceAccess: "none"` (default) uses `~/.clawdfun/sandboxes`
...
Clawdfun auto-recreates containers when `setupCommand` (or docker config) changes
unless the container was **recently used** (within ~5 minutes). Hot containers
log a warning with the exact `clawdfun sandbox recreate ...` command.
...
This builds `clawdfun-sandbox:bookworm-slim` using `Dockerfile.sandbox`.
...
This builds `clawdfun-sandbox-common:bookworm-slim`. To use it:

```json5
{
  agents: { defaults: { sandbox: { docker: { image: "clawdfun-sandbox-common:bookworm-slim" } } } }
}
```
...
This builds `clawdfun-sandbox-browser:bookworm-slim` using
...
```json5
{
  agents: {
    defaults: {
      sandbox: { browser: { image: "my-clawdfun-browser" } }
    }
  }
}
```
...
```bash
docker build -t my-clawdfun-sbx -f Dockerfile.sandbox .
```
...
```json5
{
  agents: {
    defaults: {
      sandbox: { docker: { image: "my-clawdfun-sbx" } }
    }
  }
}
```
...
- Image missing: build with [`scripts/sandbox-setup.sh`](https://github.com/clawdfun/clawdfun/blob/main/scripts/sandbox-setup.sh) or set `agents.defaults.sandbox.docker.image`.
- Container not running: it will auto-create per session on demand.
- Permission errors in sandbox: set `docker.user` to a UID:GID that matches your
  mounted workspace ownership (or chown the workspace folder).
- Custom tools not found: Clawdfun runs commands with `sh -lc` (login shell), which
