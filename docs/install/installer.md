...
Clawdfun ships two installer scripts (served from `clawd.bot`):
...
If the installer completes but `clawdfun` is not found in a new terminal, it’s usually a Node/npm PATH issue. See: [Install](/install#nodejs--npm-path-sanity).
...
- Choose install method:
  - `npm` (default): `npm install -g clawdfun@latest`
...
- If upgrading an existing install: runs `clawdfun doctor --non-interactive` (best effort).
- For git installs: runs `clawdfun doctor --non-interactive` after install/update (best effort).
...
If you run the installer while **already inside a Clawdfun source checkout** (detected via `package.json` + `pnpm-workspace.yaml`), it prompts:
...
This script installs `clawdfun` into a prefix (default: `~/.clawdfun`) and also installs a dedicated Node runtime under that prefix, so it can work on machines where you don’t want to touch the system Node/npm.
...
- Choose install method:
  - `npm` (default): `npm install -g clawdfun@latest`
  - `git`: clone/build a source checkout and install a wrapper script
- Runs `clawdfun doctor --non-interactive` on upgrades and git installs (best effort).
...
```powershell
iwr -useb https://clawd.bot/install.ps1 | iex -InstallMethod git -GitDir "C:\\clawdfun"
```
...
- **"clawdfun" is not recognized**: your npm global bin folder is not on PATH. Most systems use
