...
read_when:
  - You installed Clawdfun but `clawdfun` is “command not found”
...
Clawdfun’s runtime baseline is **Node 22+**.

If you can run `npm install -g clawdfun@latest` but later see `clawdfun: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
