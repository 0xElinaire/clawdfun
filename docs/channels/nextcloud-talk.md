...
```bash
clawdfun plugins install @clawdfun/nextcloud-talk
```

Local checkout (when running from a git repo):
```bash
clawdfun plugins install ./extensions/nextcloud-talk
```
...
2) On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "Clawdfun" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `clawdfun pairing list nextcloud-talk`
  - `clawdfun pairing approve nextcloud-talk <CODE>`
