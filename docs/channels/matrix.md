...
Matrix is an open, decentralized messaging protocol. Clawdfun connects as a Matrix **user**
...
```bash
clawdfun plugins install @clawdfun/matrix
```

Local checkout (when running from a git repo):

```bash
clawdfun plugins install ./extensions/matrix
```
...
1) Install the Matrix plugin:
   - From npm: `clawdfun plugins install @clawdfun/matrix`
   - From a local checkout: `clawdfun plugins install ./extensions/matrix`
...
3) Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Clawdfun calls the same
     login endpoint, stores the access token in `~/.clawdfun/credentials/matrix/credentials.json`,
...
- On first connection, Clawdfun requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Clawdfun logs a warning.
...
`~/.clawdfun/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `clawdfun pairing list matrix`
  - `clawdfun pairing approve matrix <CODE>`
...
- On startup, Clawdfun resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
