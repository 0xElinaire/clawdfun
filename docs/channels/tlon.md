...
Tlon is a decentralized messenger built on Urbit. Clawdfun connects to your Urbit ship and can
...
```bash
clawdfun plugins install @clawdfun/tlon
```

Local checkout (when running from a git repo):

```bash
clawdfun plugins install ./extensions/tlon
```
...
Use these with `clawdfun message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Clawdfun replies in-thread.
