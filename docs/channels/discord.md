...
## Goals
- Talk to Clawdfun via Discord DMs or guild channels.
...
3. Configure Clawdfun with `channels.discord.token` (or `DISCORD_BOT_TOKEN` as a fallback).
...
7. Direct chats: secure by default via `channels.discord.dm.policy` (default: `"pairing"`). Unknown senders get a pairing code (expires after 1 hour); approve via `clawdfun pairing approve discord <code>`.
...
This is the “Discord Developer Portal” setup for running Clawdfun in a server (guild) channel like `#help`.
...
### 2) Enable the gateway intents Clawdfun needs
...
Discord uses numeric ids everywhere; Clawdfun config prefers ids.
...
### Troubleshooting
- First: run `clawdfun doctor` and `clawdfun channels status --probe` (actionable warnings + quick audits).
...
- On startup, Clawdfun resolves channel/user names in allowlists to IDs (when the bot can search members)
  and logs the mapping; unresolved entries are kept as typed.

Native command notes:
- The registered commands mirror Clawdfun’s chat commands.
...
- Slash commands may still be visible in Discord UI to users who aren’t allowlisted; Clawdfun enforces allowlists on execution and replies “not authorized”.
...
- If the bot is stuck or rate limited, restart the gateway (`clawdfun gateway --force`) after confirming no other processes own the Discord session.
