...
- If Telegram rejects the HTML payload, Clawdfun retries the same message as plain text.

## Commands (native + custom)
Clawdfun registers native commands (like `/status`, `/reset`, `/model`) with Telegram’s bot menu on startup.
...
Notes:
- Custom commands are **menu entries only**; Clawdfun does not implement them unless you handle them elsewhere.
...
**Privacy note:** `@userinfobot` is a third-party bot. If you prefer, add the bot to the group, send a message, and use `clawdfun logs --follow` to read `chat.id`, or use the Bot API `getUpdates`.

## Config writes
By default, Telegram is allowed to write config updates triggered by channel events or `/config set|unset`.

This happens when:
- A group is upgraded to a supergroup and Telegram emits `migrate_to_chat_id` (chat ID changes). Clawdfun can migrate `channels.telegram.groups` automatically.
...
Telegram forum topics include a `message_thread_id` per message. Clawdfun:
...
Private chats can include `message_thread_id` in some edge cases. Clawdfun keeps the DM session key unchanged, but still uses the thread id for replies/draft streaming when it is present.
...
- Approve via:
  ...
  - `clawdfun pairing approve telegram <CODE>`
...
2) Run `clawdfun logs --follow` and look for `from.id`.
...
Clawdfun defaults to audio files for backward compatibility.
...
Clawdfun uses Bot API `sendMessageDraft` (not real messages) and then sends the
...
**How reactions work:**
Telegram reactions arrive as **separate `message_reaction` events**, not as properties in message payloads. When a user adds a reaction, Clawdfun:
...
**Requirements:**
- Telegram bots must explicitly request `message_reaction` in `allowed_updates` (configured automatically by Clawdfun)
...
- Example: `clawdfun message send --channel telegram --target 123456789 --message "hi"`.
...
- `clawdfun channels status` shows a warning when config expects unmentioned group messages.
- `clawdfun channels status --probe` can additionally check membership for explicit numeric group IDs (it can’t audit wildcard `"*"` rules).
...
- Check gateway logs: `clawdfun logs --follow` (look for "skipping group message")
...
- Upgrade to a Clawdfun build that normalizes abort signals, or run the gateway on Node 20 until you can upgrade.
