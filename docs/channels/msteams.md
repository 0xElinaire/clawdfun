...
```bash
clawdfun plugins install @clawdfun/msteams
```

Local checkout (when running from a git repo):
```bash
clawdfun plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
Clawdfun will offer the local install path automatically.
...
3) Configure Clawdfun with those credentials.
...
## Goals
- Talk to Clawdfun via Teams DMs, group chats, or channels.
...
- On startup, Clawdfun resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.clawdfun/clawdfun.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring Clawdfun, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field | Value |
   |-------|-------|
   | **Bot handle** | Your bot name, e.g., `clawdfun-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `clawdfun plugins install @clawdfun/msteams`
   - From a local checkout: `clawdfun plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "Clawdfun in Teams", "full": "Clawdfun in Teams" },
...
Clawdfun handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Clawdfun only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/ClawdfunShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)
Clawdfun sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `clawdfun message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.clawdfun/msteams-polls.json`.
...
**CLI:**
```bash
clawdfun message send --channel msteams \
...
clawdfun message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
clawdfun message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
clawdfun message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
clawdfun message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
