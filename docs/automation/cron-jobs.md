...
- Jobs persist under `~/.clawdfun/cron/` so restarts don’t lose schedules.
...
## Storage & history
- Job store: `~/.clawdfun/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.clawdfun/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
clawdfun cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
clawdfun cron edit <jobId> --agent ops
clawdfun cron edit <jobId> --clear-agent
```
```
...
```bash
clawdfun system event --mode now --text "Next heartbeat: check battery."
```

## Gateway API surface
- `cron.list`, `cron.status`, `cron.add`, `cron.update`, `cron.remove`
- `cron.run` (force or due), `cron.runs`
For immediate system events without a job, use [`clawdfun system event`](/cli/system).
