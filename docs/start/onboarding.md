---
summary: "First-run onboarding flow for Clawdfun (macOS app)"
read_when:
...
- Writes credentials to `~/.clawdfun/credentials/oauth.json`
...
The app can install the global `clawdfun` CLI via npm/pnpm so terminal
...
On the first agent run, Clawdfun bootstraps a workspace (default `~/clawd`):
...
```bash
clawdfun webhooks gmail setup --account you@gmail.com
```
...
- `~/.clawdfun/credentials/oauth.json`
