...
The onboarding wizard is the **recommended** way to set up Clawdfun on macOS,
...
(`web_fetch` works without a key). Easiest path: `clawdfun configure --section web`
...
1) **Existing config detection**
   - If `~/.clawdfun/clawdfun.json` exists, choose **Keep / Modify / Reset**.
   - Re-running the wizard does **not** wipe anything unless you explicitly choose **Reset**
     (or pass `--reset`).
   - If the config is invalid or contains legacy keys, the wizard stops and asks
     you to run `clawdfun doctor` before continuing.
...
2) **Model/Auth**
   ...
   - **OpenAI API key**: uses `OPENAI_API_KEY` if present or prompts for a key, then saves it to `~/.clawdfun/.env` so launchd can read it.
  ...
  - OAuth credentials live in `~/.clawdfun/credentials/oauth.json`; auth profiles live in `~/.clawdfun/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
...
5) **Channels**
  ...
  - DM security: default is pairing. First DM sends a code; approve via `clawdfun pairing approve <channel> <code>` or use allowlists.
...
7) **Health check**
   - Starts the Gateway (if needed) and runs `clawdfun health`.
   - Tip: `clawdfun status --deep` adds gateway health probes to status output (requires a reachable gateway).
...
Use `clawdfun agents add <name>` to create a separate agent with its own workspace,
...
```bash
clawdfun onboard --non-interactive \
  --mode local \
  --auth-choice apiKey \
...
- Stores it under `~/.clawdfun/tools/signal-cli/<version>/`.
...
Typical fields in `~/.clawdfun/clawdfun.json`:
...
`clawdfun agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.clawdfun/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.clawdfun/agents/<agentId>/sessions/`.
