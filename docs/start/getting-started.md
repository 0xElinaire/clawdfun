...
Recommended path: use the **CLI onboarding wizard** (`clawdfun onboard`). It sets up:
...
- **Recommended:** Brave Search API key for web search. Easiest path:
  `clawdfun configure --section web` (stores `tools.web.search.apiKey`).
...
- OAuth credentials (legacy import): `~/.clawdfun/credentials/oauth.json`
...
If you’re hacking on Clawdfun itself, run from source:
...
If `clawdfun health` shows “no auth configured”, go back to the wizard and set OAuth/key auth — the agent won’t be able to respond without it.

Tip: `clawdfun status --all` is the best pasteable, read-only debug report.
Health probes: `clawdfun health` (or `clawdfun status --deep`) asks the running gateway for a health snapshot.
