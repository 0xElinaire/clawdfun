---
summary: "Use Venice AI privacy-focused models in Clawdfun"
read_when:
  - You want privacy-focused inference in Clawdfun
...
```bash
clawdfun onboard --auth-choice venice-api-key
```
...
```bash
clawdfun chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, Clawdfun shows all available Venice models. Pick based on your needs:
...
```bash
clawdfun models set venice/claude-opus-45
clawdfun models set venice/llama-3.3-70b
```

List all available models:

```bash
clawdfun models list | grep venice
```

## Configure via `clawdfun configure`

1. Run `clawdfun configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Clawdfun automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
clawdfun chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
clawdfun chat --model venice/claude-opus-45

# Use uncensored model
clawdfun chat --model venice/venice-uncensored

# Use vision model with image
clawdfun chat --model venice/qwen3-vl-235b-a22b

# Use coding model
clawdfun chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `clawdfun models list` to see currently available models. Some models may be temporarily offline.
