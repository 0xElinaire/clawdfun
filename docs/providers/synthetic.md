---
summary: "Use Synthetic's Anthropic-compatible API in Clawdfun"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. Clawdfun registers it as the
...
```bash
clawdfun onboard --auth-choice synthetic-api-key
```
...
Note: Clawdfun's Anthropic client appends `/v1` to the base URL, so use
