---
summary: "CLI reference for `clawdfun doctor` (health checks + guided repairs)"
read_when:
...
- `--fix` (alias for `--repair`) writes a backup to `~/.clawdfun/clawdfun.json.bak` and drops unknown config keys, listing each removal.
