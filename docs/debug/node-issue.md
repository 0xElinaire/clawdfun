...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Clawdfun
---

# Node + tsx "__name is not a function" crash

## Summary
Running Clawdfun via Node with `tsx` fails at startup with:

```
[clawdfun] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `clawdfun status` and `gateway:watch` worked.
