...
Delivery:
- Publish as `@clawdfun/plugin-sdk` (or export from core under `clawdfun/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)
Scope: everything that touches core runtime behavior.
Accessed via `ClawdfunPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: ClawdfunConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: ClawdfunConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: ClawdfunConfig, agentId?: string): RegExp[];
      matchesMentionPatterns(text: string, regexes: RegExp[]): boolean;
    };
    groups: {
      resolveGroupPolicy(cfg: ClawdfunConfig, channel: string, accountId: string, groupId: string): {
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: ClawdfunConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: ClawdfunConfig): string;
  };
};
```
...
### Phase 0: scaffolding
- Introduce `@clawdfun/plugin-sdk`.
- Add `api.runtime` to `ClawdfunPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `clawdfunRuntime: ">=2026.2.0"`).
