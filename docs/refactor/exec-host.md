...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.clawdfun/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.clawdfun/node.json` (node id + pairing token).
...
Path: `~/.clawdfun/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.clawdfun/exec-approvals.sock",
...
### IPC
- Unix socket at `~/.clawdfun/exec-approvals.sock` (0600).
