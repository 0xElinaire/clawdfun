...
Clawdfun can run a **dedicated Chrome/Brave/Edge/Chromium profile** that the agent controls.
...
```bash
clawdfun browser --browser-profile clawd status
...
- `chrome`: extension relay to your **system browser** (requires the Clawdfun
...
Browser settings live in `~/.clawdfun/clawdfun.json`.
...
Clawdfun uses it automatically. Set `browser.executablePath` to override
auto-detection:
...
- **Remote CDP:** set `browser.profiles.<name>.cdpUrl` (or `browser.cdpUrl`) to
  attach to a remote Chromium-based browser. In this case, Clawdfun will not launch a local browser.
...
Clawdfun preserves the auth when calling `/json/*` endpoints and when connecting
to the CDP WebSocket. Prefer environment variables or secrets managers for
tokens instead of committing them to config files.

### Node browser proxy (zero-config default)

If you run a **node host** on the machine that has your browser, Clawdfun can
...
CDP endpoints over HTTPS. You can point a Clawdfun browser profile at a
...
clawdfun browser serve --bind <browser-host> --port 18791 --token <token>
```
...
Recommended:
- Keep `clawdfun browser serve` bound to loopback (`127.0.0.1`) and publish it via Tailscale.
...
Best practice here: keep `clawdfun browser serve` on HTTP and terminate TLS in front.
...
Clawdfun supports multiple named profiles (routing configs). Profiles can be:
...
Clawdfun can also drive **your existing Chrome tabs** (no separate “clawd” Chrome instance) via a local CDP relay + a Chrome extension.

Full guide: [Chrome extension](/tools/chrome-extension)

Flow:
- You run a **browser control server** (Gateway on the same machine, or `clawdfun browser serve`).
- A local **relay server** listens at a loopback `cdpUrl` (default: `http://127.0.0.1:18792`).
- You click the **Clawdfun Browser Relay** extension icon on a tab to attach (it does not auto-attach).
...
- “Load unpacked” → select the directory printed by `clawdfun browser extension path`
- Pin the extension, then click it on the tab you want to control (badge shows `ON`).

2) Use it:
- CLI: `clawdfun browser --browser-profile chrome tabs`
- Agent tool: `browser` with `profile="chrome"`

Optional: if you want a different name or relay port, create your own profile:

```bash
clawdfun browser create-profile \
  --name my-chrome \
  --driver extension \
...
When launching locally, Clawdfun picks the first available:
1. Chrome
2. Brave
3. Edge
4. Chromium
...
- `clawdfun browser tab`
- `clawdfun browser tab new`
- `clawdfun browser tab select 2`
- `clawdfun browser tab close 2`
...
- `clawdfun browser screenshot --ref e12`
- `clawdfun browser snapshot`
- `clawdfun browser snapshot --format aria --limit 200`
- `clawdfun browser snapshot --interactive --compact --depth 6`
- `clawdfun browser snapshot --efficient`
- `clawdfun browser snapshot --labels`
- `clawdfun browser snapshot --selector "#main" --interactive`
- `clawdfun browser snapshot --frame "iframe#main" --interactive`
- `clawdfun browser console --level error`
- `clawdfun browser errors --clear`
- `clawdfun browser requests --filter api --clear`
- `clawdfun browser pdf`
- `clawdfun browser responsebody "**/api" --max-chars 5000`

Actions:
- `clawdfun browser navigate https://example.com`
- `clawdfun browser resize 1280 720`
- `clawdfun browser click 12 --double`
- `clawdfun browser click e12 --double`
- `clawdfun browser type 23 "hello" --submit`
- `clawdfun browser press Enter`
- `clawdfun browser hover 44`
- `clawdfun browser scrollintoview e12`
- `clawdfun browser drag 10 11`
- `clawdfun browser select 9 OptionA OptionB`
- `clawdfun browser download e12 /tmp/report.pdf`
- `clawdfun browser waitfordownload /tmp/report.pdf`
- `clawdfun browser upload /tmp/file.pdf`
- `clawdfun browser fill --fields '[{"ref":"1","type":"text","value":"Ada"}]'`
- `clawdfun browser dialog --accept`
- `clawdfun browser wait --text "Done"`
- `clawdfun browser wait "#main" --url "**/dash" --load networkidle --fn "window.ready===true"`
- `clawdfun browser evaluate --fn '(el) => el.textContent' --ref 7`
- `clawdfun browser highlight e12`
- `clawdfun browser trace start`
- `clawdfun browser trace stop`

State:
- `clawdfun browser cookies`
- `clawdfun browser cookies set session abc123 --url "https://example.com"`
- `clawdfun browser cookies clear`
- `clawdfun browser storage local get`
- `clawdfun browser storage local set theme dark`
- `clawdfun browser storage session clear`
- `clawdfun browser set offline on`
- `clawdfun browser set headers --json '{"X-Debug":"1"}'`
- `clawdfun browser set credentials user pass`
- `clawdfun browser set credentials --clear`
- `clawdfun browser set geo 37.7749 -122.4194 --origin "https://example.com"`
- `clawdfun browser set geo --clear`
- `clawdfun browser set media dark`
- `clawdfun browser set timezone America/New_York`
- `clawdfun browser set locale en-US`
- `clawdfun browser set device "iPhone 14"`
...
Clawdfun supports two “snapshot” styles:

- **AI snapshot (numeric refs)**: `clawdfun browser snapshot` (default; `--format ai`)
  - Output: a text snapshot that includes numeric refs.
  - Actions: `clawdfun browser click 12`, `clawdfun browser type 23 "hello"`.
  - Internally, the ref is resolved via Playwright’s `aria-ref`.

- **Role snapshot (role refs like `e12`)**: `clawdfun browser snapshot --interactive` (or `--compact`, `--depth`, `--selector`, `--frame`)
  - Output: a role-based list/tree with `[ref=e12]` (and optional `[nth=1]`).
  - Actions: `clawdfun browser click e12`, `clawdfun browser highlight e12`.
...
- Wait for URL (globs supported by Playwright):
  - `clawdfun browser wait --url "**/dash"`
- Wait for load state:
  - `clawdfun browser wait --load networkidle`
- Wait for a JS predicate:
  - `clawdfun browser wait --fn "window.ready===true"`
- Wait for a selector to become visible:
  - `clawdfun browser wait "#main"`
...
1. `clawdfun browser snapshot --interactive`
2. Use `click <ref>` / `type <ref>` (prefer role refs in interactive mode)
3. If it still fails: `clawdfun browser highlight <ref>` to see what Playwright is targeting
...
5. For deep debugging: record a trace:
   - `clawdfun browser trace start`
   - reproduce the issue
   - `clawdfun browser trace stop` (prints `TRACE:<path>`)
...
```bash
clawdfun browser status --json
clawdfun browser snapshot --interactive --json
clawdfun browser requests --filter api --json
clawdfun browser cookies --json
```
